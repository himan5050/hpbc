...
class TCPDF {
	...
	public function Footer() {
		...
		if ($this->getRTL()) {
			...
			$this->Cell(0, 0, "This is a computer generated report, hence signature is not required. | $pagenumtxt", 'T', 0, 'L');
		} else {
			$this->SetX($ormargins['left']);
			//$this->Cell(0, 0, $pagenumtxt, 'T', 0, 'R');
			$this->Cell(0, 0, "Status as on ".date('d-m-Y')."                                                                                                                                                                              This is a computer generated report, hence signature is not required. | $pagenumtxt", 'T', 0, 'L');
		}
	}

public function pagenumber1(){
  if (empty($this->pagegroups)) {
	...
	protected function closeHTMLTagHandler(&$dom, $key, $cell, $maxbottomliney=0) {
		...
		switch($tag['value']) {
			...
			case 'table': {
				...
				if(count($dom[($dom[$key]['parent'])]['trids']) > 0){
				foreach ($dom[($dom[$key]['parent'])]['trids'] as $j => $trkey) {
					// update row-spanned cells
					if (isset($dom[($dom[$key]['parent'])]['rowspans']) && count($dom[($dom[$key]['parent'])]['rowspans']) > 0 ) {
