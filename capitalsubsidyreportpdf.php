...
<tr><td class="header1" align="left"><b>District : </b>'.ucwords(getdistrict($district)).'</td></tr>
	...
	if($fromtime !='' && $totime !=''){
		$cond .= " AND (ld.sanction_date BETWEEN '$from' AND '$to' ) ";
		$header2 .='<table cellpadding="0" cellspacing="0" border="0">
<tr><td class="header1" align="left"><b>From Date : </b>'.$fromtime.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>To Date : </b>'.$totime.'</td></tr><tr><td>&nbsp;</td></tr>
</table>';
	}else{
		if($fromtime!=''){
			$cond .= " AND ld.sanction_date='$fromtime' ";
		}
		else if($totime!=''){
			$cond .= " AND ld.sanction_date='$totime' ";
		}
	}
 ...
 while($rs = db_fetch_object($res)){
	 ...
	 if($rsh = db_fetch_object($resh)){
		...
		$bbranch = ($rs->bank_branch)?ucwords(GetBankBranch($rs->bank_branch)):'-';
				...
				<td width="11%" class="'.$class.'" align="left">'.getscheme1($rs->scheme_name).'</td>
				
				<td width="8%" class="'.$class.'" align="right">'.$bacc.'</td>
				<td width="8%" class="'.$class.'" align="left">'.ucwords($rs->fname).' '.ucwords($rs->lname).'</td>
				<td width="8%" class="'.$class.'" align="right">'.round(abs($disbursamount)).'</td>
				
				
				<td width="8%" class="'.$class.'" align="left">'.$bname.'</td>
				<td width="8%" class="'.$class.'" align="left">'.$bbranch.'</td>
