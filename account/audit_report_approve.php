...
if(isset($_POST['approve']))
{
 ...
 updateTask($adir['task_id'],$db);
  ...
  /*$awt="insert into tbl_workflow_task set level='4',status='0',doc_id='".$adr."',uid='".$_SESSION['uid']."'";
	 ...
	 createTask('4',$adr,'','',$_SESSION['uid'],$Is_escalation = '',$writ_level = '',$db);
  ...
  if($stq)
  {
   header("location:audit_report_approvelist.php?msg=Audit Approved Successfully");
  }
}
...
{  
 ...
 updateTask($adir['task_id'],$db);
  ...
  /*$awt="insert into tbl_workflow_task set level='2',status='0',doc_id='".$adr."',uid='".$_SESSION['uid']."'";
	 $awtq=DB_query($awt,$db);*/
	  createTask('2',$adr,'','',$_SESSION['uid'],$Is_escalation = '',$writ_level = '',$db);
  ...
  if($req)
  {
    header("location:audit_report_approvelist.php?msg=Query Has Been Submitted Successfully!");
  }
}
...
<table cellpadding="2" cellspacing="1" border="0" id="form-container">
  ...
  <div class="right"><textarea name="remark" cols="38"  rows="5" onkeypress="return alphanumeric(event)"></textarea></div></td></tr>
  <tr class="even">
 
	<td align="center"><input type="button" name="back" value="Back" ONCLICK="clickroute();" /> <input type="submit" name="approve" value="Approve & Forward" />     <input type="submit" value="Query" name="query" />  
	</td>
	</tr>
  </table>
</form>
...
function clickroute() {
	window.location.href='audit_report_approvelist.php';
	}
	</script>