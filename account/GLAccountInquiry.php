...
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="GLAccounts.php">GL Account Master</a> &raquo; <a href="GLAccountInquiry.php">Account Inquiry</a></div>';
if (isset($_POST['Show'])){
...
<tr><td><div class="divwrapper"><div class="maincol">'._('Account').': <span style="color:#FF0000">*</span></div><div class="maincol"><select Name="Account">';
if(!isset($_SESSION['acc']))
...
if (isset($_POST['Show'])){
   ...
   if (isset($SelectedAccount) && $SelectedAccount==''){
	...
	if ($PandLAccount==True) {
		...
		if ($RunningTotal < 0 ){ //its a credit balance b/fwd
			echo '<tr class="even">
				...
				<td>	</td>
				<td class=number align="right"><b>' . round(abs($RunningTotal)) . '</b></td>
				<td></td>
				</tr>';
			...
			if ($PeriodNo!=-9999){ //ie its not the first time around
			   ...
			   $lastdate=$lasd[2].'-'.$lasd[1].'-'.$lasd[0];
				...
				if ($PeriodTotal < 0 ){ //its a credit balance b/fwd
					...
					echo '<td></td>
						<td class=number align="right"><b>' . round(abs(-$PeriodTotal)) . '</b></td>
						<td></td>
						</tr>';
				} else { //its a debit balance b/fwd
					if ($PandLAccount==True) {
					...
					echo '<td class=number align="right"><b>' . round(abs($PeriodTotal)) . '</b></td>
						<td colspan=2></td>
						</tr>';
				}
	...
	if ($RunningTotal >0){
		echo '<td class=number align="right"><b>' . round(abs($RunningTotal)) . '</b></td><td colspan=2></td></tr>';
	}else {
		echo '<td></td><td class=number align="right"><b>' . round(abs(-$RunningTotal)) . '</b></td><td></td></tr>';
	}
	echo '</table>';
