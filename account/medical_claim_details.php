...
if($usrid==13)
{
 $ncod="and medical_claim.office='".$ofr['current_officeid']."'";	
}


$sql = "SELECT count(id ) FROM medical_claim,tbl_joinings where 1=1 ".$cond." and (status='0' or status='3') and medical_claim.emp_id=tbl_joinings.employee_id  $ncod and medical_claim.reportedto=".$usrid."";
...
$left_rec = $rec_count - ($page * $rec_limit);
 ...
 $s="select * from medical_claim,tbl_joinings where 1=1 ".$cond." and (status='0' or status='3') and medical_claim.emp_id=tbl_joinings.employee_id $ncod and medical_claim.reportedto=".$usrid." ".$orderby." LIMIT $offset, $rec_limit";
$q=DB_query($s,$db);
$nu=DB_num_rows($q);
...
?>
<div class="tblHeaderLeft"><h1>List of Claims for Approval</h1><span class="addrecord"><?php if($role != 6) echo '<a href="medical_claim_user.php">My Claims</a>'; ?></span></div><div class="tblHeaderRight"><input type="text" name="search" value="" />&nbsp;<input type="submit" name="go" value="Search" /></div>
<table>
  <tr>
  <th>S. No.</th>
  <th>Claim Id</th>
