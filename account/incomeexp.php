...
if (isset($_POST['FromPeriod']) and isset($_POST['ToPeriod']) and $_POST['FromPeriod'] > $_POST['ToPeriod']){
	...
	echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Income Expenditure</a></div><form method="POST" action="' . $_SERVER['PHP_SELF'] . '">';
...
<table align="left" class="frmtbl">
		<tr><td>
		<td><b>' . _('Select Period From :') .' <span style="color:#FF0000">*</span></b></td>		
	...
	echo '<td>
	<td><b>' . _('Select Period To :') .' <span style="color:#FF0000">*</span></b></td>
	<td>
...
foreach($acg as $acco)
{
 ...
 $sq="select gltrans.trandate,chartmaster.accountcode as accountcode,chartmaster.accountname,gltrans.amount,gltrans.trandate from chartmaster,gltrans where chartmaster.group_='".$acco."' and gltrans.account=chartmaster.accountcode and gltrans.trandate >= '".$from."' and gltrans.trandate <= '".$to."' ";
 $qq=DB_query($sq,$db);
...
/*  if(in_array($accode,$acc))
	...
	$data.= "<tr class='".$cl."'>
				...
				<td align='right'>".round(abs($qr['amount']))."</td>
		  </tr>";
		  $f++;
   }
...
$data.= "<tr class='odd'>
				...
				<td style='border-top:1px solid; border-top-color:#ccc;' align='right'><b>".round(abs($debtotal))."</td>			        </tr>";
$data.="</table>";
 $data;
...
$datae="<table><thead><tr><td align=center colspan=3><b>Expenditure</b></td></tr><th>Code</th><th>Account</th><th>Expenditure</th></thead>";
...
/*  if(in_array($accode,$acce))
	...
	$datae.= "<tr class='".$cl."'>
				...
				<td align='right'>".round(abs($qre['amount']))."</td>
		  </tr>";
		  $z++;
		}
...
$datae.= "<tr class='odd'>
				<td style='border-top:1px solid; border-top-color:#ccc;' colspan='2'><b>Total</b></td>
				<td style='border-top:1px solid; border-top-color:#ccc;' align='right'><b>".round(abs($cretotale))."</td>
		</tr>";
$datae.="</table>";

 $datae;
 $corpbranch=getCorporationBranch($_SESSION['uid'],$db);
$record="<br/><br/><table><tr class='oddrow'><td colspan='2'><h2>Income Expenditure</h2></td></tr><tr><td colspan='2' align='right'><a href='/".$u[1]."/generateaccountpdf.php?sdate=".$from."&edate=".$to."&op=incexp&branch=".$corpbranch."' target='blank'><img src='images/pdf_icon.gif'/></a></td></tr><tr><td valign='top'>".$data."</td><td valign='top'>".$datae."</td></tr></table>";
 //$record ="<table><tr><td></td></tr><tr><td></td></tr></table>";
 echo $record;

	}
	else{ 
	 if(strtotime($_POST['JournalProcessDate1']) <= strtotime($_POST['JournalProcessDate']))
	 {
