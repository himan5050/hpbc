...
/* $Id: CustomerAllocations.php 4638 2011-07-26 09:21:52Z daintree $*/
...
if (isset($_GET['AllocTrans'])) {
	...
	echo '<div class="breadcrumb">Home &raquo; Allocate Receipt</div>';
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			if ( $curDebtor != $myrow['debtorno'] ) {
				if ( $curTrans > 1 ) {
					echo '<tr class="odd">
					<td colspan="6"></td>
					<td  class="number">' . number_format($balance,2) . '</td><td><b>' . _('Balance') . '</b></td></tr>';
				}
			...
			if ( $curTrans > $trans ) {
				...
				echo '<tr class="odd">
						<td colspan="6"></td>
						<td class="number">' . number_format($balance,2) .'</td>
