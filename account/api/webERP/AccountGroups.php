...
{
  $cond.="and (sectionname like '%".$_POST['searchtext']."%' OR groupname like '%".$_POST['searchtext']."%')";
  unset($_GET{'page'});
}
	$rec_limit = 10;
...
else
{
 ...
 $left_rec = $rec_count - ($page * $rec_limit);
	 ...
	 $sql = "SELECT groupname,
		...
		FROM accountgroups
		LEFT JOIN accountsection ON sectionid = sectioninaccounts where 1=1 ".$cond."
		GROUP BY groupname LIMIT $offset, $rec_limit";
  ...
  if(isset($_GET['msg']))
  {
	...
	echo '<div class="breadcrumb">Home &raquo; <a href="AccountGroups.php">Account Groups</a></div>';
...
if($nu>0)
{
	echo '<thead><tr>
		...
		<th>' . _('Description') . '</th>
		<th colspan="3"  style="text-align:center;">' . _('Action') . '</th>
...
if (isset($_POST['SelectedAccountGroup']) OR isset($_GET['SelectedAccountGroup'])) {
		echo '<div class="breadcrumb">Home &raquo; <a href="AccountGroups.php">Account Groups</a> &raquo; <a >Edit Account Group</a></div><form method="post" id="AccountGroups" action="' . $_SERVER['PHP_SELF'] . '">';
	...
	if (isset($_GET['SelectedAccountGroup'])) {
		...
		echo '<br /><table><tr class="evenrow">';
		echo '<input  type="hidden" name="SelectedAccountGroup" value="' . $_POST['SelectedAccountGroup'] . '" />';
		echo '<td>'._('Group Name') . ':' . ' <span style="color:#FF0000">*</span></td><td>
		<input tabindex="1" ' . (in_array('GroupName',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="GroupName" size="50" maxlength="45" value="' . $_POST['GroupName'] . '" /></td></tr>';
	}
	...
	echo '<tr class="oddrow"><td><div class="left">' . _('Description') . ': <span style="color:#FF0000">*</span></div><div class="right"><textarea name="description" rows="5" cols="45" style="width: 210px; height: 48px;">' . $_POST['description'] . '</textarea></div></td></tr>';
	echo '<tr class="evenrow"><td colspan="2" align="center"><div class="centre"><a href="AccountGroups.php"><input type="button" name="back" value="Back"/></a> <input tabindex="6" type="submit" name="submit" value="' . _('Save') . '" /></div></td></tr>';
