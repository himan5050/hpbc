...
$vn=fread($file,50);
	...
	prnMsg(_('Voucher').' ' . $vn . ' '._('has been Added'),'success');
	 ...
	 $AllowThisPosting = false;
		...
		if ($AllowThisPosting ) {
			...
			if (DB_num_rows($Result)==0){
				...
				$_SESSION['JournalDetail']->add_to_glanalysis($_POST['GLAmount'], $_POST['GLNarrative'], $_POST['GLManualCode'], $myrow['accountname'], $_POST['tag'],$_POST['preparedby'],$_POST['approvedby'],$_POST['voucherno']);
			}
		}
	} else {
		$AllowThisPosting =true; //by default
		...
		if (($_POST['GLNarrative']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['GLNarrative'])))
	{
	 ...
	 $AllowThisPosting = false;
		...
		if ($AllowThisPosting ){
			...
			$Result=DB_query($SQL,$db);
			$myrow=DB_fetch_array($Result);
			$_SESSION['JournalDetail']->add_to_glanalysis($_POST['GLAmount'], $_POST['GLNarrative'], $_POST['GLCode'], $myrow['accountname'], $_POST['tag'], $_POST['preparedby'], $_POST['approvedby'],$_POST['voucherno']);
		}
	}
...
echo '<td><div class="left">' . _('Aproved By') . ': <span style="color:#FF0000">*</span></div><div class="right"><select name="approvedby">';
...
echo '<tr class="evenrow"><td><div class="left">' . _('Debit') . ': <span style="color:#0F4465">*</span></div>
				<div class="right"><input type="text" class="number" name = "Debit" onChange="eitherOr(this, '.'Credit'.')" maxlength="11" size="10" value="' . $_POST['Debit'] . '" /></div></td>';
echo '</tr><tr class="oddrow"><td><div class="left">' . _('Credit') . ': <span style="color:#0F4465">*</span></div>
...
foreach ($_SESSION['JournalDetail']->GLEntries as $JournalItem) {
	...
	echo '<td>' . $JournalItem->Narrative  . '</td><td>' . $JournalItem->preparedby  . '</td><td>' . $JournalItem->approvedby  . '</td>
