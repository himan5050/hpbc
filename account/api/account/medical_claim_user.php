...
{  
  ...
  $cond.="and (period_illness like '%".$_POST['search']."%' OR net_amount like '%".$_POST['search']."%' OR id = '".$uni[1]."')";
}

$emi="select employee_id from tbl_joinings where program_uid='".$_SESSION['uid']."'";
$emiq=DB_query($emi,$db);
$emir=DB_fetch_array($emiq);
 $sql = "SELECT count(id) as id FROM medical_claim where 1=1 ".$cond." and emp_id='".$emir['employee_id']."' and voucher_generated!='1'";
...
$left_rec = $rec_count - ($page * $rec_limit);
 ...
 $s="select * from medical_claim where 1=1 ".$cond." and emp_id='".$emir['employee_id']."' and voucher_generated!='1' ".$orderby." LIMIT $offset, $rec_limit";
$q=DB_query($s,$db);
$nu=DB_num_rows($q);
...
<div class="breadcrumb">Home &raquo; <a href="<?php echo $_SERVER['PHP_SELF'];?>">List of My Claims</a></div>
 <?php
  $i=1;
...
?>
<div class="tblHeaderLeft"><h1>My Claims</h1> <span class="addrecord"><a href="medical_claim.php">Add Claim</a></span></div><div class="tblHeaderRight"><input type="text" name="search" value="">&nbsp;<input type="submit" name="go" value="Search" /></div>
<table>
