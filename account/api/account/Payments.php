...
if (isset($_POST['CommitBatch'])){
  ...
  */
	...
	if( isset($_SESSION['clid']) && $_SESSION['type']=='loan'){
        ...
        echo '<div align="right"><a href="/'.$u[1].'/generatevoucherpdf.php?voucher=sc-'.$voucherno.'&op=payment" target="_blank"><img src="images/pdf_icon.gif"/></a></div>';
if($_SESSION['type'] == 'loan')
{
	...
	if(isset($_SESSION['clid']) && $_SESSION['type']=='loan')
	{
		...
		if (isset($LastSupplier) and $LastSupplier!='') {
			...
			echo '<br /><div align="center"><a href="' . $_SERVER['PHP_SELF'] .'">' . _('') . '<input type="button" value="Enter another General Ledger Payment"/></a></div><br />';
		}
	}
	...
	if (isset($_POST['GLNarrative'])) {
	 ...
	 if(isset($_REQUEST['GLAmount']) && $_REQUEST['type']!=''){
		...
		echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Amount') . ' (INR): <span style="color:#FF0000">*</span></div>
				<div class="right"><input type="text" name="GLAmount" Maxlength="12" size="12" class="number" value=' . $_POST['GLAmount'] . ' readonly="readonly"></div>
			</tr>';
	}
	else
	 {
