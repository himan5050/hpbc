	...
	if (isset($_GET['SelectedSectionID'])) {
		...
		echo '<table cellspacing="1" cellpadding="2">
			<tr class="evenrow">
				...
				<div class="right"><input tabindex="1" ' . (in_array('SectionID',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="SectionID" class="number" size="4" maxlength="4" value="' . $_POST['SectionID'] . '" /></div></td>
			</tr>';
	}
	echo '<tr class="oddrow"><td colspan="2"> <div class="left">' . _('Section Description') . ': <span style="color:#FF0000">*</span>' . '</div>
		<div class="right"><input tabindex="2" ' . (in_array('SectionName',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="SectionName" size="30" maxlength="200" value="' . $_POST['SectionName'] . '" onkeypress="return alphanumeric(event)"  /></div>
		</tr>';

	echo '<tr class="evenrow"><td colspan="2" align="center">';
	if (isset($_GET['SelectedSectionID'])) {
	echo '<a href="' . $_SERVER['PHP_SELF'] . '"><input tabindex="3" type="button" name="back" value="' . _('Back') . '" /></a>';
	}
