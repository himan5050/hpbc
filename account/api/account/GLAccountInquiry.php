...
if (isset($_POST['Show'])){
   ...
   if (isset($SelectedAccount) && $SelectedAccount==''){
			...
			if ($PeriodNo!=-9999){ //ie its not the first time around
			   ...
			   $lastdate=$lasd[2].'-'.$lasd[1].'-'.$lasd[0];
				...
				if ($PeriodTotal < 0 ){ //its a credit balance b/fwd
					...
					echo '<td></td>
						<td class=number align="right"><b>' . number_format(-$PeriodTotal,2) . '</b></td>
						<td></td>
						</tr>';
				} else { //its a debit balance b/fwd
					if ($PandLAccount==True) {
					...
					echo '<td class=number align="right"><b>' . number_format($PeriodTotal,2) . '</b></td>
						<td colspan=2></td>
						</tr>';
				}
	...
	if ($RunningTotal >0){
		echo '<td class=number align="right"><b>' . number_format(($RunningTotal),2) . '</b></td><td colspan=2></td></tr>';
	}else {
		echo '<td></td><td class=number align="right"><b>' . number_format((-$RunningTotal),2) . '</b></td><td></td></tr>';
	}
	echo '</table>';
