...
$title = _('Trial Balance With Budget');
...
if ((! isset($_POST['FromPeriod']) AND ! isset($_POST['ToPeriod'])) OR isset($_POST['SelectADifferentPeriod'])){
	...
	echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Trial Balance With Budget</a></div>';
	...
	<tr>	<td align="left" class="tdform-width"><fieldset><legend>Trial Balance With Budget</legend>
	...
	echo '</select></div></td><td><div class="centre"><input type=submit Name="ShowTB" Value="' . _('Generate') .'"></td>
		</tr>
		</table></fieldset>
		<br />';
...
/*Now do the posting while the user is thinking about the period to select */
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($myrow['groupname']!= $ActGrp){
			...
			if ($ActGrp !=''){
				...
				if ($myrow['parentgroupname']==$ActGrp){
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,round($GrpActual[$Level]),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($GrpBudget[$Level]),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($GrpPrdActual[$Level]),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($GrpPrdBudget[$Level]),'right');
		...
		$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,round($myrow['monthactual']),'right');
		$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($myrow['monthbudget']),'right');
		$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($AccountPeriodActual),'right');
		$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($AccountPeriodBudget),'right');
		$YPos -= $line_height;
	...
	$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,round($CheckMonth),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($CheckBudgetMonth),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($CheckPeriodActual),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($CheckPeriodBudget),'right');
...
/*Now do the posting while the user is thinking about the period to select */
	...
	echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Trial Balance</a></div>';

	/*show a table of the accounts info returned by the SQL
	Account Code ,   Account Name , Month Actual, Month Budget, Period Actual, Period Budget */
$corpbranch=getCorporationBranch($_SESSION['uid'],$db);
	echo '<table cellpadding="2"  class="listingpage_scrolltable"><tr class="oddrow"><td colspan="7"><h2>Trial Balance With Budget</h2></td></tr><tr><td colspan="7" align="right"><a href="/'.$u[1].'/generateaccountpdf.php?sdate='.$_POST['FromPeriod'].'&edate='.$_POST['ToPeriod'].'&op=trial_balance&branch='.$corpbranch.'" target="blank"><img src="images/pdf_icon.gif"/></a></td></tr>';
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
			...
			if ($ActGrp !=''){ //so its not the first account group of the first account displayed
				if ($myrow['parentgroupname']==$ActGrp){
					...
					printf('<tr>
						...
						$ParentGroups[$Level],
						round(abs($GrpActual[$Level])).$gactual,
						round(abs($GrpBudget[$Level])).$gbudget,
						round(abs($GrpPrdActual[$Level])).$gppactual,
						round(abs($GrpPrdBudget[$Level])).$gpbudget);
		...
		printf('<td>%s</td>
			...
			$ActEnquiryURL,
			$myrow['accountname'],
			round(abs($myrow['monthactual'])).$mactual,
			round(abs($myrow['monthbudget'])).$mbudget,
			round(abs($AccountPeriodActual)).$pactual,
			round(abs($AccountPeriodBudget)).$pbudget);
	...
	printf('<tr bgcolor="#ffffff">
		...
		round(abs($CheckMonth)).$cmactual,
		round(abs($CheckBudgetMonth)).$cmbudget,
		round(abs($CheckPeriodActual)).$cpactual,
		round(abs($CheckPeriodBudget)).$cpbudget);
