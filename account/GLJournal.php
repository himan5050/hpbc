...
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Journal Entry</a></div><form action="' . $_SERVER['PHP_SELF'] . '" method="post" name="form">';
if(isset($_GET['Debit']))
{
...
echo '<div class="right"><input class="number" type="text" id="GLManualCode" Name="GLManualCode" Maxlength="12" size="12" )"' .
...
$result=DB_query($sql, $db);
echo '<select name="GLCode" id="GLCode" onChange="return assignComboToInput(this,'.'GLManualCode'.')">';
...
echo '<div class="right"><input type="text" name="GLNarrative" id="GLNarrative" maxlength="200" size="100" value="'.$_POST['GLNarrative'].'" onkeypress="return alphanumeric(event)"/></div></td></tr>';
...
echo '<tr class="even">
	...
	<td class="number"><b>' . round(abs($DebitTotal)) . '</b></td>
	<td class="number"><b>' . round(abs($CreditTotal)) . '</b></td>';
	
	if ($DebitTotal!=$CreditTotal) {
	echo '<td colspan="4" align="center" style="background-color: #fddbdb"><b>' . _('Required to balance') .' - </b>' .
		round(abs($DebitTotal-$CreditTotal));
}
if ($DebitTotal>$CreditTotal) {
	echo ' ' . _('Credit') . '</td></tr>';
...
if(isset($_POST['Process']) && ($_POST['Process']==_('Accept')))
{ 

   if (($_POST['GLManualCode'] == '') ) {
			echo "<script type='text/javascript'>document.getElementById('GLManualCode').className='ercol';</script>";
		}
		if ($_POST['GLCode'] == '' ) {
