<?php
/* $Id: PricesByCost.php 4624 2011-07-05 10:36:13Z daintree $ */
...
if (isset($_POST['submit']) OR isset($_POST['update'])) {
	...
	echo '<br /><br /><form action="' . $_SERVER['PHP_SELF'] . '" method="post"><table cellspacing="1" cellpadding="2">';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	echo '<tr class="oddrow"><td colspan="2" align="center"><h2>Price By Cost</h2></td></tr><tr class="evenrow">
			<td colspan="2"> <div class="left">' . _('Category') . ':</div>
			<div class="right"><select name="StockCat">';
	echo '<option value="all">' . _('All Categories') . '';
	while ($myrow1 = DB_fetch_array($result1)) {
		echo '<option value="' . $myrow1['categoryid'] . '">' . $myrow1['categorydescription'] . '</option>';
	}
	echo '</select></div></td></tr>';
	echo '<tr class="oddrow"><td colspan="2"> <div class="left">' . _('Price') . '</div><div class="right">
				<select name="Comparator">';
	echo '<option value="1">' . _('Less than or equal to') . '</option>';
	echo '<option value="2">' . _('Greater than or equal to') . '</option>';
	if ($_SESSION['WeightedAverageCosting']==1) {
		echo '</select>'.' </div></td></tr><tr class="evenrow"><td><div class="left">'. _('Average Cost') . ' x </div><div class="right">';
	} else {
		echo '</select>'.' </div></td></tr><tr class="evenrow"><td><div class="left">'. _('Standard Cost') . ' x </div><div class="right">';
	}
	if (!isset($_POST['Margin'])){
		$_POST['Margin']=1;
	}
	echo '<div class="right"><input type="text" class="number" name="Margin" MAXLENGTH =8 size=8 value=' .$_POST['Margin'] . '></div></td></tr>';
	$result = DB_query("SELECT typeabbrev, sales_type FROM salestypes", $db);
	echo '</tr><tr class="oddrow"><td colspan="2"> <div class="left">' . _('Sales Type') . '/' . _('Price List') . ':</div>
		<div class="right"><select name="SalesType">';
	while ($myrow = DB_fetch_array($result)) {
	...
	echo '</select></div></td></tr>
		<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Currency') . ':</div>
	...
	DB_data_seek($result, 0);
	echo '</select></td></tr><tr class="oddrow"><td colspan="2" align="center"><input type="submit" name="submit" value="' . _('Submit') . '"></td></tr>
	</table>';
	//echo '<br /><p><div class="centre"><input type="submit" name="submit" value="' . _('Submit') . '"></div></p>';
