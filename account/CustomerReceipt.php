...
/* $Id: CustomerReceipt.php 4618 2011-07-02 23:04:59Z daintree $ */

include('includes/DefineReceiptClass.php');
include('includes/session.inc');

$title = _('Cash Receipt Voucher');

include('includes/header.inc');
include('includes/SQL_CommonFunctions.inc');
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Receipt Entry</a></div>';
$msg='';
?>
<script type="text/javascript">
function clickroute() {
	window.location.href='CustomerReceipt.php?NewReceipt=Yes&Type=GL';
	}
</script>
<?php
//print_r($_SESSION['ReceiptBatch']);
//echo "test".$_POST['BatchNarrative'];
...
$_POST['Amount'] = isset($_GET['Amount'])?$_GET['Amount']:$_POST['Amount'];
...
if ((isset($_POST['Process']) and $_POST['BankAccount']!=''))
{
...
$_SESSION['ReceiptBatch']->Narrative = ucwords($_POST['Narrative']);
	...
	$_SESSION['ReceiptBatch']->Narrative = ucwords($_POST['Narrative']);
	$_SESSION['ReceiptBatch']->BatchNarrative=ucwords($_POST['BatchNarrative']);
...
if (isset($_POST['Process']) && $_POST['BankAccount']!=''){ //user hit submit a new entry to the receipt batch
    ...
    if ($btr['type']=='Cheque' && $_POST['cheque']=='') {
		prnMsg( _('Enter Cheque Detail'),'warn');
		$BankAccountEmpty=TRUE;
		
	}
	 if ($btr['type']=='Cheque' && $_POST['cheque']!='') {
	 if((strlen($_POST['cheque'])<6) || !is_numeric($_POST['cheque']))
		prnMsg( _('Enter Correct Cheque Detail'),'warn');
		$BankAccountEmpty=TRUE;
	}
	 if ($_POST['Amount'] == '' ) {
	 ...
	 if (($_POST['Amount']!='') && (!eregi('^([0-9]+(\.[0-9]+)?$)' , $_POST['Amount'])))
	{
	//  $BankAccountEmpty=TRUE;
     prnMsg(_('Enter Valid Value For Amount'),'error');
	}
	 if (($_POST['Amount']!='') && ($_POST['Amount'])<=0)
	{
	 // $BankAccountEmpty=TRUE;
     prnMsg(_('Enter Correct Amount'),'error');
	}

	
	  if ($btr['type']=='Cheque' && $_POST['PayeeBankDetail']=='') {
		prnMsg( _('Enter Payee Bank Detail'),'warn');
		//$BankAccountEmpty=TRUE;
	}
	 
	 if (($_POST['PayeeBankDetail']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['PayeeBankDetail'])))
	{
	// $BankAccountEmpty=TRUE;
     prnMsg(_('Enter valid Name for Payee Bank detail, A-Z or 0-9 is Allowed'),'error');
	}
     if ($_POST['Narrative'] == '' ) {
		prnMsg( _('Enter Narrative'),'warn');
	//	$BankAccountEmpty=TRUE;
	}
	 if (($_POST['Narrative']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['Narrative'])))
	{
	 $BankAccountEmpty=TRUE;
    // prnMsg(_('Enter valid Narrative, A-Z or 0-9 is Allowed'),'error');
	 ...
	 if (($_POST['BatchNarrative']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['BatchNarrative'])))
	{
	 $BankAccountEmpty=TRUE;
     prnMsg(_('Enter valid Narrative, A-Z or 0-9 is Allowed'),'error');
	}
	...
	else if (($_POST['Amount']!='') && (!eregi('^([0-9]+(\.[0-9]+)?$)' , $_POST['Amount'])))
	{
	  $BankAccountEmpty=TRUE;
    // prnMsg(_('Enter Valid Value For Amount'),'error');
	}
	...
	/*else if(($_POST['Amount']!='') && ($_POST['Amount'])>0)
	{
	  $amo=@explode('.',$_POST['Amount']);
	 
	  if((strlen($amo[1])>2) || (strlen($amo[0])>8))
	  {
	    $BankAccountEmpty=TRUE;
         prnMsg(_('Enter valid Amount, Like xxxxxxxx.xx'),'error');
	  }
	}*/
	
	  else if ($btr['type']=='Cheque' && $_POST['PayeeBankDetail']=='') {
	//	prnMsg( _('Enter Payee Bank Detail'),'warn');
		$BankAccountEmpty=TRUE;
	}
	 
	else if (($_POST['PayeeBankDetail']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['PayeeBankDetail'])))
	{
	 $BankAccountEmpty=TRUE;
    // prnMsg(_('Enter valid Name for Payee Bank detail, A-Z or 0-9 is Allowed'),'error');
	}
    else if ($_POST['Narrative'] == '' ) {
	//	prnMsg( _('Enter Narrative'),'warn');
		$BankAccountEmpty=TRUE;
	}
	 else if (($_POST['Narrative']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['Narrative'])))
	{
	 $BankAccountEmpty=TRUE;
    // prnMsg(_('Enter valid Narrative, A-Z or 0-9 is Allowed'),'error');
	}
...
if (isset($Cancel)){
	...
	unset($_SESSION['ReceiptBatch']->Account);
	//unset($_SESSION['paytype']);
...
$file=fopen("voucher.txt","r");
	...
	echo '<table><thead>
				...
				<th>' . _('GL Code') . '</th>
				<th>' . _('Amount of Receipt').'</th></thead>';
	...
	foreach ($_SESSION['ReceiptBatch']->Items as $ReceiptItem) {
		...
		echo '<td>'.$_SESSION['ReceiptBatch']->DateBanked.'</td>
				<td>'.$ReceiptItem->GLCode.' - '.$myrow['accountname'].'</td>
				<td class=number>'.round(abs($ReceiptItem->Amount)) .'</td>';
...
//
			...
			$BatchDebtorTotal += (($ReceiptItem->Discount + $ReceiptItem->Amount));
		...
		if ($BatchReceiptsTotal!=0){
			...
			$SQL="INSERT INTO gltrans (type,
				...
				VALUES (
					...
					'" . ucwords($_SESSION['ReceiptBatch']->Narrative) . "',
					'" . $_SESSION['UsersRealName'] . "',
					'" . $_POST['approvedby'] . "',
					'" . $BatchReceiptsTotal . "',
					'".$_SESSION['ReceiptBatch']->cheque."',
					'".$vn."',
					'" . $_SESSION['paytype'] . "'
				)";
	...
	$voucherno=$vouchno[1];
	  ...
	  fclose($file1);
prnMsg( _('Voucher') . ' ' . "sc-".($vounumber-1) . ' ' . _('has been successfully entered into the database'),'success');

echo '<div align="right"><a href="/'.$u[1].'/generatevoucherpdf.php?voucher='.$voucherno.'&op=cash" target="_blank"><img src="images/pdf_icon.gif"/></a></div>';

echo '<br /><div align="center"><input type="button" value="Enter another Receipt Voucher" ONCLICK="clickroute();"/></div><br />';
if(isset($_GET['clid']) && $_GET['clid'])
{
	echo '&nbsp;&nbsp;<div align="center"><a href="/'.$u[1].'/loan/pendingvouchers">' . _('') . '<input type="button" value="Back To Pending Vouchers"/></div>';
}
	$vg ="update tbl_pendingvouchers set pendingstatus='1' ,voucher_number = 'sc-".($vounumber-1)."' where id='".$_SESSION['clid']."'";
	$vgq=DB_query($vg,$db);
	if(isset($_SESSION['clid']) && $_SESSION['type']=='loan')
	{
		...
		if($trid[1] == 'One Time Settlement')
			$tbl = 'tbl_loan_repayment';
		if($trid[1] == 'Promoter Share')
			$tbl = 'tbl_loan_repayment';
		if($tbl)
		{
		...
		unset($_SESSION['clid']);
		unset($_SESSION['type']);
		//$_SESSION['message'] = "The receipt voucher is successfully posted";
		//header("location:/scst/loan/pendingvouchers");exit;
	}
...
<tr class="oddrow"><td align="center" ><h2> Receipt</h2></td></tr>

<tr class="evenrow"><td><div class="left">' . _('Cash/Bank Account') . ': <span style="color:#FF0000">*</span></div>
				 <div class="right"><select tabindex=1 name="BankAccount" id="BankAccount" onChange="chequeno(this.value)" >';

if (DB_num_rows($AccountsResults)==0){
	echo '</select></div></td></tr></table><p>';
	...
	while ($myrow=DB_fetch_array($AccountsResults)){
		...
		if (($_SESSION['ReceiptBatch']->Account==$myrow['accountcode']) || ($_POST['BankAccount']==$myrow['accountcode'])){
...
echo '<tr class="oddrow"><td><div class="left">' . _('Date') . ': <span style="color:#FF0000">*</span></div>
		<div class="right"><input tabindex=2 type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="DateBanked" maxlength=10 size=10 onChange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" value="' . $_SESSION['ReceiptBatch']->DateBanked . '"><input tabindex=5 type="hidden" name="FunctionalExRate" class=number maxlength=10 size=12 value="' . $_SESSION['ReceiptBatch']->FunctionalExRate . '"></div></td></tr>';
/*echo '<tr class="evenrow"><td><div class="left"></div>
		<div class="right"><input type="hidden" value="INR" name="Currency" />';
...
if ($_SESSION['ReceiptBatch']->AccountCurrency!=$_SESSION['ReceiptBatch']->Currency AND isset($_SESSION['ReceiptBatch']->AccountCurrency)){
	if (isset($SuggestedExRate)){
		$SuggestedExRateText = '<b>' . _('Suggested rate:') . ' ' . round($SuggestedExRate,2) . '</b>';
	} else {
		$SuggestedExRateText ='';
	}
...
if ($_SESSION['ReceiptBatch']->AccountCurrency!=$_SESSION['CompanyRecord']['currencydefault']
	...
	if (isset($SuggestedFunctionalExRate)){
		$SuggestedFunctionalExRateText = '<b>' . _('Suggested rate:') . ' ' . round($SuggestedFunctionalExRate,2) . '</b>';
	} else {
	...
	/*echo '<tr><td></td><td><input tabindex=5 type="hidden" name="FunctionalExRate" class=number maxlength=10 size=12 value="' . $_SESSION['ReceiptBatch']->FunctionalExRate . '"></td>
			<td></td></tr>';*/
}
//echo '<tr class="oddrow"><td>' . _('Receipt Type') . ':</td><td><select tabindex=6 name="ReceiptType">';
...
<div class="right"><input type="text" class="number" name="cheque" id="cheque" maxlength="6" size="10" value="' . $_POST['cheque'] . '" /> </div>' . _('') . '</td>
	</tr>';
echo '<tr class="evenrow"><td><div class="left">' . _('Prepared By') . ': <span style="color:#FF0000">*</span></div>
		<div class="right"><input type="text" tabindex=3 name="preparedby" maxlength=45 size=82 value="' . $_SESSION['UsersRealName'] . '" readonly /></div></td>
		</tr>';
		echo '<tr class="oddrow"><td><div class="left">'. _('Approved By').': <span style="color:#FF0000">*</span></div><div class="right"><select name="approvedby" tabindex=4>';
...
echo '<tr class="evenrow"><td><div class="left">' . _('Narrative') . ':</div><div class="right"><input type="text" id="BatchNarrative" name="BatchNarrative" value="'. $_SESSION['ReceiptBatch']->BatchNarrative . '" onkeypress="return alphanumeric(event)" tabindex=5/> </div></td></tr>';
//echo '<tr class="oddrow"><td colspan=3 align="center"> <div class="centre"><input tabindex=8 type=submit name="BatchInput" Value="' . _('Accept') . '"></div></td></tr>';
echo '</table><br />';
...
if (isset($_SESSION['ReceiptBatch'])){
	...
	foreach ($_SESSION['ReceiptBatch']->Items as $ReceiptItem) {
		...
		echo '<tr class="even">
				<td >' . round(abs($ReceiptItem->Amount)) . '</td>
				
				<td>'.$ReceiptItem->GLCode.' - '.$myrow['accountname'].'</td>
				<td style="max-width:70px; word-wrap:break-word;">'.ucwords($ReceiptItem->Narrative) . '</td>
				<td><a href="' . $_SERVER['PHP_SELF'] . '?Delete=' . $ReceiptItem->ID . '&Type=' . $_GET['Type']. '">' . _('Delete') . '</a></td>
			</tr>';
		$BatchTotal= $BatchTotal + $ReceiptItem->Amount;
	}

	echo '<tr ><td colspan="6"><b>' . round(abs($BatchTotal)) . '</b></td></tr></table>';
}
...
/*a customer is selected  */
	...
	echo '<tr>
		<td class=number>' . round(abs($_SESSION['CustomerRecord']['balance'])) . '</td>
		<td class=number>' . round(abs(($_SESSION['CustomerRecord']['balance'] - $_SESSION['CustomerRecord']['due']))) . '</td>
		<td class=number>' . round(abs(($_SESSION['CustomerRecord']['due']-$_SESSION['CustomerRecord']['overdue1']))) . '</td>
		<td class=number>' . round(abs(($_SESSION['CustomerRecord']['overdue1']-$_SESSION['CustomerRecord']['overdue2']))) . '</td>
		<td class=number>' . round(abs($_SESSION['CustomerRecord']['overdue2'])) . '</td>
		</tr>
		</table><br />';

	echo '<table class="selection">';

	$DisplayDiscountPercent = round($_SESSION['CustomerRecord']['pymtdiscount']*100,2) . '%';
...
/* Set up a heading for the transaction entry for a GL Receipt */
	echo '<br /><table cellspacing="1" cellpading="2">';
	echo '<tr class="oddrow"><td colspan="2" align="center"><h2>' . _('Cash Receipt Voucher') . '</h2></td></tr>';

	//Select the tag
	/*echo '<tr class="evenrow"><td>' . _('Select Tag') . ':</td><td><select name="tag">';
...
// End select tag
	...
	echo '<tr class="evenrow"><td><div class="left">' . _('GL Account') . ': <span style="color:#FF0000">*</span></div><div class="right"><select tabindex=8 name="GLCode" id="GLCode">';
	$SQL = 'SELECT accountcode, accountname FROM chartmaster ORDER BY accountcode';
	$result=DB_query($SQL,$db);
	if (DB_num_rows($result)==0){
		echo '</select></div>' . _('No General ledger accounts have been set up yet') . ' - ' . _('receipts cannot be entered against GL accounts until the GL accounts are set up') . '</td></tr>';
	} else {
		echo '<option value="">--Select--</option>';
		while ($myrow=DB_fetch_array($result)){
...
if (((isset($_SESSION['CustomerRecord'])
	...
	echo '<tr class="oddrow"><td><div class="left">' . _('Amount of Receipt') . ': <span style="color:#FF0000">*</span></div>
		<div class="right"><input tabindex="9" type="text" name="Amount" maxlength="11" size="13" class="number" value="' . $_POST['Amount'] . '" id="Amount" ></div></td>
	</tr>';

	if (!isset($_POST['GLEntry'])){
		echo '<tr class="evenrow"><td><div class="left">' . _('Amount of Discount') . ': <span style="color:#FF0000">*</span></div>
			<div class="right"><input tabindex="10" type="text" name="Discount" maxlength=12 size=13 class="number" value="' . $_POST['Discount'] . '"> ' . _('agreed prompt payment discount is') . ' ' . $DisplayDiscountPercent . '</div></td></tr>';
	} else {
		echo '<input tabindex=11 type="hidden" name="Discount" Value=0>';
	}

	echo '<tr class="evenrow"><td><div class="left">' . _('Payee Bank Details') . ': <span style="color:#FF0000">*</span></div>
		<div class="right"><input tabindex=12 type="text" id="PayeeBankDetail" name="PayeeBankDetail" maxlength=45 size=20 value="' . $_POST['PayeeBankDetail'] . '"onkeypress="return alphanumeric(event)"></div></td></tr>';
//	echo '<tr><td>' . _('Narrative') . ':</td>
//		<td><input tabindex=13 type="text" name="Narrative" maxlength=30 size=32 value="' . $_POST['Narrative'] . '"></td></tr>';
	echo '<tr class="oddrow" ><td><div class="left">' . _('Narrative') . ': <span style="color:#FF0000">*</span></div>';
	echo '<div class="right"><input type="text" id="Narrative" tabindex=13 name="Narrative" maxlength="200" size="82" value="'.$_POST['Narrative'].'" onkeypress="return alphanumeric(event)"/></div></td></tr>';
	
	echo '
	<tr class="evenrow"><td colspan="2" align="center">
	...
	if (isset($CustomerSearchResult)) {
		...
		while ($myrow=DB_fetch_array($CustomerSearchResult)) {
			...
			printf('<td><font size=1>
					<input tabindex='.round(12+$j).' type=submit name="Select" value="%s"></font></td>
					<td>%s</td></tr>',
					$myrow['debtorno'],
					$myrow['name']);
...
if (isset($_SESSION['ReceiptBatch']->Items) and count($_SESSION['ReceiptBatch']->Items) > 0){
	echo '<br /><div class="centre" align="center"><input tabindex="13" type="submit" name="CommitBatch" value="' . _('Accept and Process Batch') . '"></div>';
}
echo '</form>';
include('includes/footer.inc');
?>
...
if (isset($_POST['Process']) ) {
	...
	if($_POST['BankAccount']=='')
	{
	echo "<script type='text/javascript'>document.getElementById('BankAccount').className='ercol';</script>";
	}
	if($_POST['BankAccount']=='')
	{
	echo "<script type='text/javascript'>document.getElementById('GLCode').className='ercol';</script>";
	}
	if($_POST['Amount']=='')
	{ 
	echo '<style>.datee {border:1px solid red !important}</style>';
	echo "<script type='text/javascript'>document.getElementById('Amount').style.border='1px solid red !important';</script>";
	echo "<script type='text/javascript'>document.getElementById('Amount').className='number';</script>";
	
	}
	if($_POST['PayeeBankDetail']=='')
	{
	echo "<script type='text/javascript'>document.getElementById('PayeeBankDetail').className='ercol';</script>";
	}
		/*if (($_POST['BatchNarrative']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['BatchNarrative'])))
	{
	  echo "<script type='text/javascript'>document.getElementById('BatchNarrative').className='ercol';</script>";
	}*/
	 if (($_POST['BatchNarrative']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['BatchNarrative'])))
	{
	...
	if($_POST['Narrative']=='')
	{
	echo "<script type='text/javascript'>document.getElementById('Narrative').className='ercol';</script>";
	}
	if($_POST['cheque']=='')
	{
	echo "<script type='text/javascript'>document.getElementById('cheque').className='ercol';</script>";
	}
	if($_POST['GLCode']=='')
	{
	echo "<script type='text/javascript'>document.getElementById('GLCode').className='ercol';</script>";
