...
/* $Id: GLProfit_Loss.php 4630 2011-07-14 10:27:29Z daintree $*/
...
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Profit and Loss Report</a></div>';
	...
	echo '<p><table  border="0" cellpadding="2" cellspacing="1" style="border:none;">
 <tr>	<td align="left" class="tdform-width"><fieldset><legend>Profit and Loss Report</legend>
  <table align="left" class="frmtbl">
			<tr><td><div class="maincol">' . _('Select Period From') . ':</div>
	...
	echo '</select><input type="hidden" name="Detail" value="Detailed"></div></td>';

	echo '<td align="right"><input type="submit" name="ShowPL" value="' ._('Generate') . '">&nbsp;&nbsp;
				...
				if ($myrow['parentgroupname']!=$ActGrp){
					while ($myrow['groupname']!=$ParentGroups[$Level] AND $Level>0) {
						...
						if ($Section == 1){ /*Income */
							...
							$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round(-$GrpPrdActual[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round(-$GrpPrdBudget[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round(-$GrpPrdLY[$Level]),'right');
							$YPos -= (2 * $line_height);
						} else { /*Costs */
							$LeftOvers = $pdf->addTextWrap($Left_Margin +($Level*10),$YPos00 -($Level*10),$FontSize,$ActGrpLabel);
							$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($GrpPrdActual[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($GrpPrdBudget[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($GrpPrdLY[$Level]),'right');
							$YPos -= (2 * $line_height);
...
// Print heading if at end of page
					...
					if ($Section == 1){ /*Income */
						$LeftOvers = $pdf->addTextWrap($Left_Margin +($Level*10),$YPos00 -($Level*10),$FontSize,$ActGrpLabel); $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round(-$GrpPrdActual[$Level]),'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round(-$GrpPrdBudget[$Level]),'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round(-$GrpPrdLY[$Level]),'right');
						$YPos -= (2 * $line_height);
					} else { /*Costs */
		...
		if ($myrow['sectioninaccounts'] != $Section){
			...
			if ($Section != ''){
				...
				if ($Section == 1) { /*Income*/
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos00,$FontSize,$Sections[$Section]);
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round(-$SectionPrdActual),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round(-$SectionPrdBudget),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round(-$SectionPrdLY),'right');
					$YPos -= (2 * $line_height);
				...
				if ($Section == 2){ /*Cost of Sales - need sub total for Gross Profit*/
					$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos00,$FontSize,_('Gross Profit'));
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($TotalIncome - $SectionPrdActual),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($TotalBudgetIncome - $SectionPrdBudget),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($TotalLYIncome - $SectionPrdLY),'right');
					$pdf->line($Left_Margin+310, $YPos+$line_height,$Left_Margin+500, $YPos+$line_height);
					$pdf->line($Left_Margin+310, $YPos,$Left_Margin+500, $YPos);
					$YPos -= (2 * $line_height);
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos00,$FontSize,_('Gross Profit Percent'));
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($PrdGPPercent,1) . '%','right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($BudgetGPPercent,1) . '%','right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($LYGPPercent,1). '%','right');
					$YPos -= (2 * $line_height);
				}
			}
			$SectionPrdLY = 0;
			...
			if ($_POST['Detail'] == 'Detailed'){
				$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos00,$FontSize,$Sections[$myrow['sectioninaccounts']]);
				$YPos -= (2 * $line_height);
			}
			$FontSize =8;
		...
		if ($myrow['groupname'] != $ActGrp){
			...
			if ($_POST['Detail'] == 'Detailed'){
				$FontSize =10;
				$pdf->setFont('','B');
				$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos00,$FontSize,$myrow['groupname']);
				$YPos -= (2 * $line_height);
				$FontSize =8;
				$pdf->setFont('','');
			}
		...
		if ($_POST['Detail'] == 'Detailed') {
			...
			if ($Section == 1) { /*Income*/
				$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round(-$AccountPeriodActual),'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round(-$AccountPeriodBudget),'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round(-$AccountPeriodLY),'right');
			} else {
				$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($AccountPeriodActual),'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($AccountPeriodBudget),'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($AccountPeriodLY),'right');
	...
	if ($Section != ''){
		...
		if ($Section == 2){ /*Cost of Sales - need sub total for Gross Profit*/
			...
			$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round(100*($TotalIncome - $SectionPrdActual)/$TotalIncome,1) . '%','right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round(100*($TotalBudgetIncome - $SectionPrdBudget)/$TotalBudgetIncome,1) . '%','right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round(100*($TotalLYIncome - $SectionPrdLY)/$TotalLYIncome,1). '%','right');
			$YPos -= (2 * $line_height);
		}
	}

	$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,60,$FontSize,_('Profit').' - '._('Loss'));
	$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round(-$PeriodProfitLoss),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round(-$PeriodBudgetProfitLoss),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round(-$PeriodLYProfitLoss),'right');
	$YPos -= (2 * $line_height);
	...
	$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,round($PrdPLPercent,1) . '%','right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,round($BudgetPLPercent,1) . '%','right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,round($LYPLPercent,1). '%','right');
	$YPos -= (2 * $line_height);
   ...
   $numr=DB_num_rows($AccountsResult);
	echo '<div class="breadcrumb">Home &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Profit and Loss Report</a></div>';
...
{ $corpbranch=getCorporationBranch($_SESSION['uid'],$db);
	echo '<table cellpadding=2 class=selection><tr class="oddrow"><td colspan="8"><h2>Profit and Loss Report</h2></td></tr><tr><td colspan="8" align="right"><a href="/'.$u[1].'/generateaccountpdf.php?op=profitloss&periodfrom='.$_POST['FromPeriod'].'&periodto='.$_POST['ToPeriod'].'&Detail=Detailed&earning='.$RetainedEarningsAct.'&branch='.$corpbranch.'" target="_blank"><img src="images/pdf_icon.gif"/></a></td></tr><tr class="evenrow"><td colspan="8"><b>'. _('Statement of Profit and Loss for the') . ' ' . $NumberOfMonths . ' ' . _('months to') . ' and including ' . $PeriodToDate . '</b></td></tr>';

	if ($_POST['Detail']=='Detailed'){
		$TableHeader = '<tr>
							<th>' . _('Account') . '</th>
							<th>'._('Account Name') .'</th>
							<th colspan=2>' . _('Current Year') .'</th>
							<th colspan=2>' . _('Period Budget') .'</th>
							<th colspan=2>' . _('Previous Year') . '</th>
						</tr>';
	} else { /*summary */
		$TableHeader = '<tr>
							<th colspan=2></th>
		...
		if ($myrow['groupname']!= $ActGrp){
			if ($myrow['parentgroupname']!=$ActGrp AND $ActGrp!=''){
					...
					if ($_POST['Detail']=='Detailed'){
						...
						$ActGrpLabel = str_repeat('',$Level) . $ParentGroups[$Level] . ' ' . _('total');
					} else {
				...
				if ($Section ==1){ /*Income */
						...
						printf('<tr>
								...
								<td></td>
								<td class=number align="right">%s</td>
								<td></td>
								<td class=number align="right">%s</td>
								<td></td>
								<td class=number align="right">%s</td>
								...
								round(-$GrpPrdActual[$Level]),
								round(-$GrpPrdBudget[$Level]),
								round(-$GrpPrdLY[$Level]));
					} else { /*Costs */
		...
		if ($myrow['sectioninaccounts']!= $Section){
			...
			if ($SectionPrdLY+$SectionPrdActual+$SectionPrdBudget !=0){
				if ($Section==1) { /*Income*/
					...
					printf('<tr>
							...
							$Sections[$Section],
							round(-$SectionPrdActual),
							round(-$SectionPrdBudget),
							round(-$SectionPrdLY));
							$TotalIncome = -$SectionPrdActual;
							$TotalBudgetIncome = -$SectionPrdBudget;
							$TotalLYIncome = -$SectionPrdLY;
				...
				if ($Section==2){ /*Cost of Sales - need sub total for Gross Profit*/
					...
					printf('<tr>
							...
							round($TotalIncome - $SectionPrdActual),
							round($TotalBudgetIncome - $SectionPrdBudget),
							round($TotalLYIncome - $SectionPrdLY));
					...
					printf('<tr>
							<td colspan=2><font size=2>'._('Gross Profit Percent').'</font></td>
							<td></td>
							<td class=number align="right">%s</td>
							<td></td>
							<td class=number align="right">%s</td>
							...
							round($PrdGPPercent,1) . '%',
							round($BudgetGPPercent,1) . '%',
							round($LYGPPercent,1). '%');
					$j++;
				}
			}
			...
			if ($_POST['Detail']=='Detailed'){
				printf('<tr class="oddrow">
					<td colspan=8 align="center"><b>%s</b></td>
					</tr>',
		...
		if ($_POST['Detail']=='Detailed'){
			...
			if ($Section ==1){
				 printf('<td>%s</td>
						...
						$ActEnquiryURL,
						$myrow['accountname'],
						round(-$AccountPeriodActual),
						round(-$AccountPeriodBudget),
						round(-$AccountPeriodLY));
			} else {
				printf('<td>%s</td>
						<td>%s</td>
						<td class=number align="right">%s</td>
						<td></td>
						<td class=numbe align="right"r>%s</td>
						<td></td>
						<td class=number align="right">%s</td>
						<td></td>
						</tr>',
	...
	if ($myrow['sectioninaccounts']!= $Section){
		...
		if ($_POST['Detail']=='Detailed' and isset($Sections[$myrow['sectioninaccounts']])){
			printf('<tr>
				<td colspan=6><font size=4 color=BLACK><b>%s</b></font></td>
				</tr>',
				$Sections[$myrow['sectioninaccounts']]);
		}
		$j++;
	...
	printf('<tr bgcolor="#ffffff">
			<td colspan=2><font size="4" ><b>'._('Profit').' - '._('Loss').'</b></font></td>
			<td></td>
			<td class="number" align="right">%s</td>
			<td></td>
			<td class="number" align="right">%s</td>
			...
			round(-$PeriodProfitLoss),
			round(-$PeriodBudgetProfitLoss),
			round(-$PeriodLYProfitLoss)
			);
	...
	printf('<tr>
			<td colspan="2"><font size="2">'._('Net Profit Percent').'</font></td>
			<td></td>
			<td class="number" align="right">%s</td>
			<td></td>
			<td class="number" align="right">%s</td>
			...
			round($PrdNPPercent,1) . '%',
			round($BudgetNPPercent,1) . '%',
			round($LYNPPercent,1). '%');
	...
	echo '<br /><div align="center"><input type="submit" name="SelectADifferentPeriod" value="' . _('Select A Different Period') . '" /></div>';
