...
/* $Id: StockAdjustments.php 4632 2011-07-16 02:59:14Z daintree $*/
...
echo '<tr class="oddrow"><td align="center"><h2>'._('Adjustment Details').'</h2></td></tr>';
if (!isset($_GET['Description'])) {
	$_GET['Description']='';
}

echo '<tr class="evenrow"><td ><div class="left">'. _('Stock Code'). ':</div><div class="right"><select name="StockID"><option value="">--Select--</option>';
$sc="select * from item_master order by name";
$scq=DB_query($sc,$db);
while($scr=DB_fetch_array($scq))
{
  if ($StockID==$scr['code']) {
	echo '<option value="'.$scr['name'].'" selected>'.$scr['name'].'</option>';
} else {
	echo '<option value="'.$scr['name'].'" >'.$scr['name'].'</option>';
}
}
echo '</select></div></td></tr>';

echo '<tr class="oddrow"><td><div class="left">'. _('Partial Description'). ':</div><div class="right"><input type="text" name="StockText" size=21 value="' .
	$_GET['Description'] .'"></div></td></tr><tr class="evenrow"><td><div class="left">'._('Partial Stock Code'). ':</div><div class="right">';
if (isset($StockID)) {
...
echo '</div></td></tr><tr class="oddrow"><td><div class="left"><input type=submit name="CheckCode" VALUE="'._('Check Part').'"></td></tr>';
if (isset($_SESSION['Adjustment']) and mb_strlen($_SESSION['Adjustment']->ItemDescription)>1){
	echo '<tr class="even"><td >' . $_SESSION['Adjustment']->ItemDescription . ' ('._('In Units of').' ' .
		$_SESSION['Adjustment']->PartUnit . ' ) - ' . _('Unit Cost').' = ' .
			number_format($_SESSION['Adjustment']->StandardCost,4) . '</div></td></tr>';
}

echo '<tr class="oddrow"><td><div class="left">'. _('Adjustment to Stock At Location').':</div><div class="right"><select name="StockLocation"> ';
...
echo '<tr class="evenrow"><td><div class="left">'. _('Comments On Why').':</div>
	<div class="right"><input type="text" name="Narrative" size=32 maxlength=30 value="' . $_SESSION['Adjustment']->Narrative . '"></div></td></tr>';

echo '<tr class="oddrow"><td><div class="left">'._('Adjustment Quantity').':</div>';
...
echo '</div></td></tr>';
	//Select the tag
echo '<tr class="oddrow"><td><div class="left">'._('Select Tag').'</div><div class="right"><select name="tag">';
...
echo '<tr class="evenrow" align="center"><td><input type=submit name="EnterAdjustment" value="'. _('Enter Stock Adjustment'). '"></td></tr></table><br />';
