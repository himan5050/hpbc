...
function clickroute() {
	window.location.href='Payments.php?NewPayment=Yes';
	}
</script>
<?php
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Payment Entry</a></div>';
...
if(isset($_GET['type']))
{
 ...
 if($_POST['BankAccount'] && $_POST['BankAccount']!='')
...
if ($btr['type']=='Cheque' && $_POST['cheque']=='' && isset($_POST['Process']) ) {
	prnMsg(_('Enter Cheque Number'), 'warn');
	$BankAccountEmpty=TRUE;
}
if ($btr['type']=='Cheque' && $_POST['cheque']!='' && isset($_POST['Process'])) {
	 if((strlen($_POST['cheque'])<6) || !is_numeric($_POST['cheque']))
		 {
     prnMsg(_('Cheque Number Must Be 6 Character Long'),'error');
	 $BankAccountEmpty=TRUE;
		 }
	}
	  if (isset($_POST['Process']) AND $_POST['GLManualCode']=='')
   {
	    prnMsg( _('Enter GL Account Manually Or Select GL Account') ,'warn');
   }
	if ($_POST['Narrative']!='' && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['Narrative']))) {
	 
     prnMsg(_('Enter Valid Refrence/Narrative, A-Z or 0-9 is Allowed'),'error');
	 $BankAccountEmpty=TRUE;
...
if(isset($_POST['GLNarrative']) AND $_POST['GLNarrative']!='' AND !isset($_POST['Cancel']) AND isset($_POST['Process']) AND (!eregi('^[0-9a-zA-Z ]+$' , $_POST['GLNarrative']))) {
prnMsg(_('Enter Valid Narrative, A-Z or 0-9 is Allowed'), 'warn');
	$BankAccountEmpty=TRUE;
}
...
if (isset($_POST['Narrative']) AND $_POST['Narrative']!=''){
	$_SESSION['PaymentDetail']->Narrative=ucwords($_POST['Narrative']);
}
...
if (isset($_POST['CommitBatch'])){
  ...
  */
	...
	// it is a supplier payment by cheque and haven't printed yet so print cheque
		...
		//its a nominal bank transaction type 1
			...
			foreach ($_SESSION['PaymentDetail']->GLItems as $PaymentItem) {
				...
				*/
					if($TrfToBankExRate)
						$exrate = (($_SESSION['PaymentDetail']->ExRate * $_SESSION['PaymentDetail']->FunctionalExRate)/$TrfToBankExRate);
					else
						$exrate = 0;
	...
	else if( isset($_SESSION['clid']) && $_SESSION['type']=='billsubmit')
	{
	  ...
	  /*Its a supplier payment type 22 */
			...
			$SQL = $SQL . "valueS ('" . $TransNo . "',
					...
					'" . ucwords($_SESSION['PaymentDetail']->Narrative) . "'
				)";
	...
	if( isset($_SESSION['clid']) && $_SESSION['type']=='loan'){
		...
		prnMsg(_('Payment') . ' ' . "sc-".($vounumber-1) . ' ' . _('has been successfully entered'),'success');
        echo '<div align="right"><a href="/'.$u[1].'/generatevoucherpdf.php?voucher='.($vounumber-1).'&op=payment" target="_blank"><img src="images/pdf_icon.gif"/></a></div>';
if(isset($_GET['clid']) && $_GET['clid'])
{
	...
	if(isset($_SESSION['clid']) && $_SESSION['type']=='loan')
	{
		...
		unset($_SESSION['clid']);
		unset($_SESSION['type']);
		//$_SESSION['message'] = "The payment voucher is successfully posted";
		...
		if (isset($LastSupplier) and $LastSupplier!='') {
			...
			echo '<br /><div align="center"><input type="button" value="Enter another General Ledger Payment" ONCLICK="clickroute();"/></div></div><br />';
		}
	}
...
echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Cash/Bank Account') . ': <span style="color:#FF0000">*</span></div>
		<div class="right"><select name="BankAccount" onChange="chequeno(this.value)" id="BankAccount">';
...
echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Reference / Narrative') . ': </div>
		<div class="right"><input type="text" name="Narrative" id="Narrative" maxlength="200"  size=82 value="' . $_POST['Narrative'] . '"onkeypress="return alphanumeric(event)"/></div></td>
		</tr>';
...
/* Set upthe form for the transaction entry for a GL Payment Analysis item */
	...
	if (isset($_POST['GLManualCode'])) {
		echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Enter GL Account Manually') . ': <span style="color:#FF0000">*</span></div>
			<div class="right"><input type=Text class="number" id="GLManualCode" Name="GLManualCode" maxlength=12 size=12  value="'. $_POST['GLManualCode'] .'"  ></div></td></tr>';
	} else {
		echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Enter GL Account Manually') . ': <span style="color:#FF0000">*</span></div>
			<div class="right"><input type=Text class="number" id="GLManualCode" Name="GLManualCode" Maxlength=12 size=12 onChange="return assignComboToInput(this,'.'GLCode'.')" ></div></tr>';
	}
	
	echo '<tr class="oddrow"><td colspan="2"> <div class="left">' . _('Select GL Group') . ': </div>
		<div class="right"><select name="GLGroup" id="GLGroup">';
	...
	echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Select GL Account') . ': <span style="color:#FF0000">*</span></div>
		<div class="right"><select name="GLCode" id="GLCode" onChange="return assignComboToInput(this,'.'GLManualCode'.')">';
	...
	if (isset($_POST['GLNarrative'])) {
		echo '<tr class="oddrow"><td colspan="2"> <div class="left">' . _('GL Narrative') . ': <span style="color:#FF0000">*</span></div>
				<div class="right"><input type="text" name="GLNarrative" id="GLNarrative" maxlength="200" size="52" value="' . $_POST['GLNarrative'] . '" onkeypress="return alphanumeric(event)"/>
				</div></td>
			</tr>';
	} else {
		echo '<tr class="oddrow"><td colspan="2"> <div class="left">' . _('GL Narrative') . ': <span style="color:#FF0000">*</span></div>
				<div class="right"><input type="text" id="GLNarrative" name="GLNarrative" maxlength="200" size="52" onkeypress="return alphanumeric(event)"/></div></td>
			</tr>';
	}

	 if(isset($_REQUEST['GLAmount']) && $_REQUEST['type']!=''){
		...
		echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Amount') . ' (INR): <span style="color:#FF0000">*</span></div>
				<div class="right"><input type="text" name="GLAmount" id="GLAmount" Maxlength="12" size="12" class="number" onkeypress="return paypaymain_custom(event,"GLAmount",11)" value=' . $_POST['GLAmount'] . ' readonly="readonly"></div>
			</tr>';
	}
	else
	 {
		//echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Amount') . ' (' . $_SESSION['PaymentDetail']->Currency . '):</div>';
		 ...
		 echo '<tr class="evenrow"><td colspan="2"> <div class="left">' . _('Amount') . ' ( INR ): <span style="color:#FF0000">*</span></div>
				<div class="right"><input type="text" name="GLAmount" id="GLAmount" maxlength="11" size="12" class="number" value=' . $_POST['GLAmount'] . '></div></td>
			</tr>';
	}
	...
	if (sizeOf($_SESSION['PaymentDetail']->GLItems)>0) {
		...
		foreach ($_SESSION['PaymentDetail']->GLItems as $PaymentItem) {
			...
			echo '<tr class="odd">
				...
				<td class=number>' . round($PaymentItem->Amount) . '</td>
				<td>' . $PaymentItem->GLCode . ' - ' . $PaymentItem->GLActName . '</td>
				<td>' . ucwords(stripslashes($PaymentItem->Narrative))  . '</td>
		...
		echo '<tr class="total"><td style="border:none;"></td><td class="number" colspan="4"><b>' . round($PaymentTotal) . '</b></td></tr></table><br />';
...
if(isset($_POST['Process'])){
	if(count($_SESSION['PaymentDetail']->GLItems)==0)
	{ //user hit submit a new GL Analysis line into the payment

	 if ($_POST['GLAmount'] == '') {
			/*echo "<script type='text/javascript'>document.getElementById('GLAmount').className='ercol';</script>";*/
			echo "<script type='text/javascript'>document.getElementById('GLAmount').style.border='1px solid red !important';</script>";
	      echo "<script type='text/javascript'>document.getElementById('GLAmount').className='number';</script>";
	}  
	...
	echo "<script type='text/javascript'>document.getElementById('BankAccount').className='ercol';</script>";
	  ...
	  if ($_POST['GLManualCode']=='') {
		  echo "<script type='text/javascript'>document.getElementById('GLManualCode').style.border='1px solid #ff0000';</script>";
	echo "<script type='text/javascript'>document.getElementById('GLManualCode').className='number';</script>";
	
      }
	   if ($_POST['GLNarrative']=='') {
	echo "<script type='text/javascript'>document.getElementById('GLNarrative').className='ercol';</script>";
	...
	if ($btr['type']=='Cheque' && $_POST['cheque']=='') {
	   echo "<script type='text/javascript'>document.getElementById('cheque').style.border='1px solid #ff0000';</script>";
		echo "<script type='text/javascript'>document.getElementById('cheque').className='number';</script>";
	}
	if ($_POST['Narrative']!='' && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['Narrative']))) {
