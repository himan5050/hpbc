	...
	//echo '<pre>';
	  //print_r($_POST);exit;
		if (isset($_POST['Clear_'.$Counter]) AND $_POST['Clear_'.$Counter]==True){  
			...
			$AmountCleared = round($myrow[0] / $myrow[1]);
...
{
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'?Type=Payments">Bank Payment Reconcillation</a></div>';
}
else if($_REQUEST['Type']=='Receipts')
{
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'?Type=Receipts">Bank Receipt Reconcillation</a></div>';
}
...
echo '<tr class="oddrow"><td ><div class="left">' . _('Show') . ' ' . $TypeName . ' ' . _('from') . ':</div>
		<div class="right"><input tabindex="2" type="text" name="AfterDate" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" size="12" maxlength="10" onChange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" value="' . $_POST['AfterDate'] . '"></div></td></tr>';

echo '<tr class="evenrow"><td><div class="left">' . _('To') . ':</div>
	<div class="right"><input tabindex="3" type="text" name="BeforeDate" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" size="12" maxlength="10" onChange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" Value="' . $_POST['BeforeDate'] . '"></div></td></tr>';
...
//echo 'aa'.$SQLBeforeDate;
   ...
   $numr=DB_num_rows($PaymentsResult);
	$TableHeader = '<tr><th>'. _('S. No.'). '</th>
	                 <th>'. _('Refrence/Narrative'). '</th>
						<th>' . $TypeName . '</th>
