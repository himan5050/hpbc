...
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="'.$_SERVER['SCRIPT_NAME'].'">Audit Schedule Report</a></div>';
...
<table  border="0" cellpadding="2" cellspacing="1" style="border:none;">
 <tr>	<td align="left" class="tdform-width"><fieldset>
 <legend>Audit Schedule Report</legend>
...
if($sdate=='')
 ...
 if($InputError!=1)
	...
	$corpbranch=getCorporationBranch($_SESSION['uid'],$db);
	  $output="<table><tr class=oddrow><td colspan='5'><h2>Audit Schedule Report</h2></td></tr><tr><td colspan='5' align='right'><a href='/".$u[1]."/generateaccountpdf.php?sdate=".strtotime($sdate)."&edate=".strtotime($edate)."&office=".$_REQUEST['auditoffice']."&op=auditschedule&branch=".$corpbranch."' target='blank'><img src='images/pdf_icon.gif'/></a></td></tr>
	  <tr><th  width='10%'><b>S. No.</b></th><th><b>Office Name</b></th><th><b>Auditor</b></th><th><b>Audit Start Date</b></th><th><b>Period</b></th></tr>";
	  $s="select * from audit_plan where startdate>='".strtotime($sdate)."' and startdate<='".strtotime($edate)."' and auditoffice='".$_REQUEST['auditoffice']."' order by startdate";
	  $q=DB_query($s,$db);
	  $k=1;
	  ...
	  {  if($k%2==0)
	   ...
	   $output .="<tr class='".$cl."'><td valign='top' width='10%' align='center'>".$k."</td><td valign='top'>".ucwords($r2['corporation_name'])."</td><td valign='top'>".ucwords($r1['name'])."</td><td valign='top' align='center'>".date('d-m-Y',$r['startdate'])."</td><td valign='top'>".$r['period']."</td></tr>";
	   	$k++;	   
	  }
	$output .="</table>";
	echo $output;
