...
<tr><td class="header_first" align="left">
 ...
 $k=1;
	  ...
	  { $date=$r['trandate'];
		...
		$approvedby=$r['approvedby'];
		   ...
		   $r1=db_fetch_array($q1);
	    	   if($r['amount'] >0)
			   { $deb=$deb+$r['amount'];
	            $outputd .='<tr><td valign="top"  class="'.$cl.'">Dbt. '.$r1['accountname'].'</td><td valign="top"  class="'.$cl.'">'.$r['amount'].'</td><td valign="top"  class="'.$cl.'">&nbsp;</td></tr>';
	             }
				 else if($r['amount'] <0)
				 {$cre=$cre+$r['amount'];
				 $outputd .='<tr><td valign="top"  class="'.$cl.'">Cr. '.$r1['accountname'].'</td><td valign="top"  class="'.$cl.'">&nbsp;</td><td valign="top"  class="'.$cl.'">'.$r['amount'].'</td></tr>';
				 }
	   	$k++;	   
	}
	...
	$output .='</table><table><tr><td colspan="2"><b>Payment Voucher No: </b>'.$voucher.'</td><td colspan="2" align="right"><b>Date: </b>'.$dat[2].'-'.$dat[1].'-'.$dat[0].'</td></tr>
	  <tr></tr></table><br>
	  <table border="1" style="border:1px solid #ccc;"><tr><td width="50%"><b>Particulars</b></td><td width="25%"><b>Debit</b></td><td width="25%"><b>Credit</b></td></tr>'.$outputd.'<tr><td></td><td  class="header4_2"><b>'.$deb.'</b></td><td class="header4_2"><b>'.$cre.'</b></td></tr>
	  <tr><td colspan="3">Rupees : '.convert_number($deb).' Only</td></tr></table><br><br>
	  <table border="0">
	  <tr><td>Prepared By </td><td>Manager </td><td>G.M.</td><td>M.D.</td></tr>
	  <tr><td>&nbsp;</td></tr>
	   <tr><td colspan="4">Received a sum of Rs '.$deb.' (Rupees '.convert_number($deb).' Only) from H.P.S.C.S.T. on account of...... </td></tr>
	  </table>';
...
$SQL = "SELECT accountgroups.groupname,
	...
	while ($myrow=db_fetch_array($AccountsResult)) {
		...
		if ($myrow['groupname']!= $ActGrp ){
			if ($ActGrp !=''){ //so its not the first account group of the first account displayed
				if ($myrow['parentgroupname']==$ActGrp){
					...
					$output .='<tr>
						<td align="right" colspan="2"><b>'.$ParentGroups[$Level].' Total </b></td><td class="number"><b>'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</b></td><td></td>
					...
					do {
						$output .='<tr>
							...
							<td class="number" align="right"><b>'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</b></td>
					...
					if ($Level>0){
						$output .='<tr>
						<td colspan="2"><b>'.$ParentGroups[$Level].' Total </b></td>
						<td class="number" colspan="2" align="right"><b>'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</b></td>
		...
		if((number_format($AccountPeriodActual,$_SESSION['CompanyRecord']['decimalplaces']))>0)
		   { 
		     $actd=number_format($AccountPeriodActual,$_SESSION['CompanyRecord']['decimalplaces']) ;
			 $actc="0.00" ;
			} 
		else if ((number_format($AccountPeriodActual,$_SESSION['CompanyRecord']['decimalplaces']))<0)
		   { 
		     $actc=number_format($AccountPeriodActual,$_SESSION['CompanyRecord']['decimalplaces']) ;
			 $actd="0.00";
		   }
		   else
		   {
...
if($j%2==0)
	...
	if ($ActGrp !=''){ //so its not the first account group of the first account displayed
		if ($myrow['parentgroupname']==$ActGrp){
			...
			$output .='<tr>
				...
				<td class="number" colspan="2">'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</td>
			...
			do {
				$output .='<tr>
					...
					<td align="right"><b>'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</b></td>
					<td></td>
					</tr>';
			...
			if ($Level >0){
				...
				<td  colspan="2">'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</td>
	...
	$output .='<tr bgcolor="#ffffff">
			...
			<td class="header2" align="right">'.round($CheckPeriodActuald,2).'</td>
			<td class="header2" align="right">'.round($CheckPeriodActualc,2).'</td>
