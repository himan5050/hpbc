...
<td class="header2"><b>Balance Amount</b></td>
	 ...
	 else
	 {
	   $cl="header4_2";
	    ...
	    $output .='<tr ><td class="'.$cl.'" align="center">'.$i.'</td><td class="'.$cl.'">'.ucwords($secr['lookup_name']).'</td><td align="right" class="'.$cl.'"></td><td align="right" class="'.$cl.'">'.($r['amount']).'</td></tr>';
		$i++;
	  }
	
	  
	  $output .='<tr><td colspan="3" class="header2"><b>Total</b></td><td align="right" class="header2"><b>'.$totbal.'</b></td></tr></table>';
...
<b>Section : </b>'.ucwords($sect).'</td></tr>
	   ...
	   if($_POST['empname'])
	   {
	     $cond1 ="and ( tbl_joinings.employee_name like '".$_POST['empname']."' )";
		 $cond .=",tbl_joinings where 1=1 ".$cond1."";
	   }else{
		    $cond .= ", tbl_joinings where 1=1";
		   }
	    ...
	    if($_POST['section'])
	   {
		 ...
		 //$tbl=",tbl_lookups";
		 $con=" and tbl_joinings.program_uid=loanadvance.empid";
	   }
	   $totbal=0;
	   $totadv=0;
	   
   $s="select * from loanadvance $cond";
	$q=db_query($s);
...
<td class="header2"><b>Balance Amount</b></td>
	 ...
	 else
	 {
	   $cl="header4_2";
		...
		<td align="right" class="'.$cl.'"></td><td align="right" class="'.$cl.'">'.($r['amount']).'</td></tr>';
		$i++;
	  }
	
	  
	  $output .='<tr><td colspan="4" class="header2"><b>Total</b></td><td class="header2"></td><td align="right"  class="header2"><b>'.$totbal.'</b></td></tr></table>';
...
<td class="header2"><b>Bill Paid</b></td>
	 ...
	 else
	 {
	   $cl="header4_2";
	    ...
	    $output .='<tr ><td class="'.$cl.'" align="center">'.$i.'</td><td class="'.$cl.'">'.ucwords($r['name']).'</td><td align="left" class="'.$cl.'"><div style="text-transform:uppercase">'.ucwords($r['refnum']).'</div></td><td align="center" class="'.$cl.'">'.date('d-m-Y',$r['date']).'</td><td align="right" class="'.$cl.'">'.$r['amount'].'</td><td class="'.$cl.'">'.$sta.'</td></tr>';
		$i++;
	  }
