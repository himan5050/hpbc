...
$cl="header4_2";
  ...
  /*$data.= '<tr class="'.$cl.'">';
	...
	$inco[$accou][$mo]=	round($account[$acid]['de'],2); 
}
     $inco['Total Cash In'][$mo]=round(array_sum($totaldebit),2); 
...
/*$data .='<tr class="'.$cl.'">';
	...
	$expo[$accoue][$mo]=round(abs($accounte[$acide]['cr']),2); 
}
$expo['Total Cash Out'][$mo]=round(array_sum($totalcredite),2); 
...
$dat .='<td>'.$data.'</td>';*/
 ...
 $expo['Monthly Cash Flow (Cash In - Cash Out)'][$mo]=round((array_sum($totaldebit))-(array_sum($totalcredite)),2);
 $expo['Begining Cash Balance'][$mo]=round($opening,2);
 $expo['Ending Cash Balance'][$mo]=round(($cico+$opening),2);
