...
<?php
echo '<div class="breadcrumb"><a href="/'.$u[1].'">Home</a> &raquo; <a href="AccountGroups.php">Accounts Groups</a> &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Add Account Group</a></div>';
...
if (isset($_POST['submit'])) {
  ...
  if (mb_strlen($_POST['GroupName'])==0){
		$InputError = 1;
		prnMsg( _('Enter Account Group Name'),'error');
		$Errors[$i] = 'GroupName';
		$i++;
	}
	...
	if ($_POST['SequenceInTB']=='') {
		$InputError = 1;
		prnMsg( _('Enter Sequence In The TB'),'error');
		$Errors[$i] = 'SectionInAccounts';
		$i++;
	}
	...
	if ($_POST['SequenceInTB']!='' && !is_numeric($_POST['SequenceInTB']) or $_POST['SequenceInTB'] > 10000) {
		$InputError = 1;
...
if (! isset($_GET['delete'])) {
	...
	if (isset($_GET['SelectedAccountGroup'])) {
		...
		<input tabindex="1" ' . (in_array('GroupName',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="GroupName" id="GroupName" size="50" maxlength="45" value="' . $_POST['GroupName'] . '" onkeypress="return alphanumeric(event)" onkeypress = "return fononlyn(event)" /></div></td></tr>';
	}
	echo '<tr class="oddrow"><td><div class="left">' . _('Parent Group') . ':' . ' <span style="color:#FF0000">*</span></div>
			<div class="right"><select tabindex="2" ' . (in_array('ParentGroupName',$Errors) ?  'class="selecterror"' : '' ) . '  name="ParentGroupName" id="ParentGroupName">';
	...
	<div class="right"><select tabindex="3" ' . (in_array('SectionInAccounts',$Errors) ?  'class="selecterror"' : '' ) .
      '  name="SectionInAccounts" id="SectionInAccounts">';
	...
	echo '<div class="right"><input tabindex="5" ' . (in_array('SequenceInTB',$Errors) ? 'class="inputerror"' : '' ) .
		' type="text" maxlength="5" name="SequenceInTB" id="SequenceInTB" class="number" value="' . $_POST['SequenceInTB'] . '" onkeypress = "return fononlyn(event)" /></div></td></tr>';
	echo '<tr class="oddrow"><td><div class="left">' . _('Description') . ': <span style="color:#FF0000">*</span></div><div class="right"><input type="text" name="description" value="' . $_POST['description'] .'" id="description" maxlength="200" onkeypress="return alphanumeric(event)" tabindex="6" /></div></td></tr>';
	echo '<tr class="evenrow"><td align="center"><div class="centre"><input  type="button" tabindex="7" value="Back" ONCLICK="clickroute();" /> <input tabindex="8" type="submit" name="submit" value="' . _('Save') . '" /></div></td></tr>';
...
if (isset($_POST['submit'])) {
  ...
  if (mb_strlen($_POST['GroupName'])==0){
		echo "<script type='text/javascript'>document.getElementById('GroupName').className='ercol';</script>";
		$Errors[$i] = 'GroupName';
		$i++;
	}
	 if (($_POST['GroupName']!='') && (!eregi('^[0-9a-zA-Z ]+$' , $_POST['GroupName'])))
	{
	 ...
	 if ($_POST['SectionInAccounts']==''){
		echo "<script type='text/javascript'>document.getElementById('SectionInAccounts').className='ercol';</script>";
		$Errors[$i] = 'GroupName';
		$i++;
	}
	...
	if ($myrow[0]!=0 and $_POST['SelectedAccountGroup']=='') {
		echo "<script type='text/javascript'>document.getElementById('SelectedAccountGroup').className='ercol';</script>";
		$Errors[$i] = 'GroupName';
		$i++;
	}
	if (ContainsIllegalCharacters($_POST['GroupName'])) {
	...
	if ($_POST['ParentGroupName'] !=''){
		if (CheckForRecursiveGroup($_POST['GroupName'],$_POST['ParentGroupName'],$db)) {
			echo "<script type='text/javascript'>document.getElementById('ParentGroupName').className='ercol';</script>";
			$Errors[$i] = 'ParentGroupName';
			$i++;
		}
	}
