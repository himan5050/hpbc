...
/* $Id: GLBalanceSheet.php 4618 2011-07-02 23:04:59Z daintree $*/
...
/*	echo '<div class="page_help_text">'
	...
	echo '<p><table border="0" cellpadding="2" cellspacing="1" style="border:none;">
	 <tr>	<td align="left" class="tdform-width"><fieldset><legend>Balance Sheet</legend>
	<table align="left" class="frmtbl">
		<tr><td align="left" ><div class="maincol" style="width: 155px;"><b>'._('Select the balance date').':</b></div><div class="maincol"><select Name="BalancePeriodEnd">';
	...
	/*echo '<tr class="oddrow"><td>'._('Detail Or Summary').':</td><td><select Name="Detail">';
	echo '<option selected VALUE="Summary">'._('Summary') . '</option>';
	echo '<option selected VALUE="Detailed">'._('All Accounts') . '</option>';
	echo '</select></td></tr>';*/
    echo '<td colspan="2" align="center">
	<input type=submit Name="ShowBalanceSheet" Value="'._('Generate').'">&nbsp;&nbsp;
...
$bald=explode('/',$BalanceDate);
   // echo '<a href="/'.$u[1].'/generateinventorypdf.php?op=balancesheet&period='.$_POST['BalancePeriodEnd'].'&Detail=Detailed&earning='.$RetainedEarningsAct.'" target="_blank">';
   $corpbranch=getCorporationBranch($_SESSION['uid'],$db);
	echo '<div class=invoice><table class="selection"><tr><th colspan=6><div class="centre"><b>' .
		_('Balance Sheet as at') . ' ' . $bald[2].'-'.$bald[1].'-'.$bald[0] .'</b></div></th></tr><tr><td colspan="6" align="right"><a href="/'.$u[1].'/generateaccountpdf.php?op=balancesheet&period='.$_POST['BalancePeriodEnd'].'&Detail=Detailed&earning='.$RetainedEarningsAct.'&branch='.$corpbranch.'" target="_blank"><img src="images/pdf_icon.gif"/></a></td></tr>';
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($myrow['sectioninaccounts']!= $Section ){
			...
			if ($_POST['Detail']=='Detailed'){
				printf('<tr>
					<td colspan=6><font size=4 ><b>%s</b></font></td>
					</tr>',
		...
		if ($myrow['groupname']!= $ActGrp){
			...
			if ($_POST['Detail']=='Detailed'){
				...
				<td colspan=6 align="center"><b>%s</b></td>
				</tr>',
				$myrow['groupname']);
				echo $TableHeader;
			}
	...
	printf('<tr>
		<td colspan=2><b>'._('Check Total').'</b></font></td>
		<td class=number>%s</td>
		<td></td>
		<td class=number>%s</td>
		</tr>',
