...
//header close
  ...
  $sql = "SELECT tbl_headmaster.name1,tbl_headmaster.vid,tbl_scheme_master.scheme_name,tbl_headmaster.createdon,tbl_headmaster.code,tbl_loan_detail.loan_requirement, 
		 ...
		 ,tbl_loanee_detail.reg_number,tbl_loanee_detail.fname,tbl_loanee_detail.lname,tbl_loanee_detail.fh_name , tbl_loanee_detail.address1,tbl_loanee_detail.address2	
...
$header3 .='<table cellpadding="2" cellspacing="2" border="0" class="tbl_border" align="center" width="1200px;"><tr>
<td width="3%" class="header2">S. No.</td>
<td width="5%" class="header2">Date</td>
<td width="7%" class="header2">Registration No.</td>
...
where 1=1 AND tbl_scheme_master.loan_type=148";
 ...
 while($rs = db_fetch_object($res)){
	...
	$loan +=$loan_term;
				...
				<td width="7%" class="'.$class.'" align="right">'.round(abs($disamount)).'</td>
				<td width="7%" class="'.$class.'" align="right">'.round(abs($rs->promoter_share)).'</td>
				<td width="5%" class="'.$class.'" align="right">'.round(abs($loan_term)).'</td>
				<td width="6%" class="'.$class.'" align="right">'.round(abs($rs->apex_share)).'</td>
				<td width="6%" class="'.$class.'" align="right">'.round(abs($rs->corp_share)).'</td>
 ...
 $output .= '<tr>
					  <td width="3%" class="'.$class.'" align="center">&nbsp;</td>
					  <td width="5%" class="'.$class.'" align="center">&nbsp;</td>
					  <td width="7%" class="'.$class.'" align="right">Total</td>
					  <td width="7%" class="'.$class.'" align="right">'.round($dist).'</td>
					  <td width="7%" class="'.$class.'" align="right">'.round($promoter).'</td>
					  <td width="5%" class="'.$class.'" align="right">'.round($loan).'</td>
					  <td width="6%" class="'.$class.'" align="right">'.round($apex).'</td>
					  <td width="6%" class="'.$class.'" align="right">'.round($corp).'</td>
					  <td width="8%" class="'.$class.'" align="left">&nbsp;</td>
