...
$cond = '';	
	...
	if($case_no){
		$cond .= " AND tbl_courtcasehearing.courtcase_id lIKE '%$case_no%'";
...
<tr><td class="header1" align="left">Case Number : '.ucwords($case_no).'</td></tr>
	...
	if($court_name_name){
		$cond .= " AND tbl_court_names.court_name_name lIKE '%$court_name_name%' ";
		$header2 .='<table cellpadding="0" cellspacing="0" border="0">
<tr><td class="header1" align="left">Court Name : '.ucwords($court_name_name).'</td></tr>
</table><br />';
	}
	
	if($lawyer_name){
		$cond .= " AND tbl_lawyer.lawyer_name lIKE '%$lawyer_name%' ";
		$header2 .='<table cellpadding="0" cellspacing="0" border="0">
...
<tr><td class="header1" align="left">Loan Account No : '.ucwords($loan_account).'</td></tr>
	...
	if($fromtime!='' && $totime!=''){
		$cond .= " AND(tbl_courtcasehearing.date BETWEEN '$fromtime' AND '$totime') ";
		$header2 .='<table cellpadding="0" cellspacing="0" border="0">
<tr><td class="header1" align="left"><b>From Date : </b>'.$fromtime.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>To Date : </b>'.$totime.'</td></tr><tr><td>&nbsp;</td></tr>
</table><br />';
	}else{
		if($fromtime!=''){
			$cond .= " AND tbl_courtcasehearing.date='$fromtime' ";
		}
		if($totime!=''){
			$cond .= " AND tbl_courtcasehearing.date='$totime' ";
		}
	}
...
<td width="8%" colspan="1" align="center" class="header2">Case Number</td>
...
<td width="8%" colspan="1" align="center" class="header2">Loan Account Number</td>
...
INNER JOIN tbl_court_names ON(tbl_courtcasehearing.court_name_id=tbl_court_names.court_name_id)
 ...
 while($rs = db_fetch_object($res)){
  $sd=$rs->hearing_date;
	 $dsd=substr($sd,0,10);
	  $hearingdate ="";
	 $sqlh = "select hearing_date,current_hearing_date,status from tbl_courtcase where case_no='".$rs->courtcase_id."'";
	 $resh = db_query($sqlh);
	 while($rsh = db_fetch_object($resh)){
	     $hearingdate .= date('d-m-Y',strtotime($rsh->hearing_date)).'<br />';
	     $comment .=$rsh->current_hearing_date.'<br />';
	     $statust .=$rsh->status.'<br />';
	 }
   ...
   if($counter%2==0){ $class='header4_1';}else{$class='header4_2';}
				...
				<td width="8%" class="'.$class.'" align="center">'.$hearingdate.'</td>
				<td width="8%" class="'.$class.'" align="left">'.$rs->fee_charge.'</td>
				<td width="8%" class="'.$class.'" align="left">'.ucwords($statust).'</td>
