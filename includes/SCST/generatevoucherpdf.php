...
<tr><td class="header_first" align="left">
 ...
 $k=1;
	...
	$output .='</table><table><tr><td colspan="2"><b>Payment Voucher No: </b>'.$voucher.'</td><td colspan="2" align="right"><b>Date: </b>'.$dat[2].'-'.$dat[1].'-'.$dat[0].'</td></tr>
	  ...
	  <tr><td colspan="3">Rupees : '.convert_number($deb).'</td></tr></table><br><br>
	  <table border="0">
	  <tr><td>Prepared By </td><td>Manager </td><td>G.M.</td><td>M.D.</td></tr>
	  <tr><td>&nbsp;</td></tr>
	   <tr><td colspan="4">Received a sum of Rs '.$deb.' (Rupees '.convert_number($deb).' ) from H.P.S.C.S.T. on account of...... </td></tr>
	  </table>';
...
$output .=' <table cellspacing="0" cellpadding="0" border="0">
	...
	$dat=explode('-',$date);
	$output .='</table><table><tr><td colspan="2"><b>Cash Voucher No: </b>'.$voucher.'</td><td colspan="2" align="right"><b>Date: </b>'.$dat[2].'-'.$dat[1].'-'.$dat[0].'</td></tr>
	  <tr></tr></table><br>
	  <table border="1" style="border:1px solid #ccc;"><tr><td width="50%"><b>Particulars</b></td><td width="25%"><b>Debit</b></td><td width="25%"><b>Credit</b></td></tr>'.$outputd.'<tr><td></td><td  class="header4_2"><b>'.$deb.'</b></td><td class="header4_2"><b>'.$cre.'</b></td></tr>
	  <tr><td colspan="3">Rupees : '.convert_number($deb).'</td></tr></table><br><br>
	 <table border="0">
		...
		if ($myrow['groupname']!= $ActGrp ){
			if ($ActGrp !=''){ //so its not the first account group of the first account displayed
				if ($myrow['parentgroupname']==$ActGrp){
					...
					$output .='<tr>
						<td colspan="2">'.$ParentGroups[$Level].' Total </td><td class="number" colspan="2"><I>'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</I></td>
					...
					do {
						$output .='<tr>
							...
							<td class="number" colspan="2"><I>'.number_format($GrpPrdActual[$Level],$_SESSION['CompanyRecord']['decimalplaces']).'</I></td>
...
if($j%2==0)
	...
	else
	{
	 $cl="header4_2";
		...
		$output .='<tr ><td class="'.$cl.'">'.$ActEnquiryURL.'</td><td class="'.$cl.'">'.$myrow['accountname'].'</td><td class="'.$cl.'">'. $actd.'</td><td class="'.$cl.'">'. $actc.'</td></tr>';
	...
	$output .='<tr bgcolor="#ffffff">
			...
			<td class="header2">'.number_format($CheckPeriodActuald,$_SESSION['CompanyRecord']['decimalplaces']).'</td>
			<td class="header2">'.number_format($CheckPeriodActualc,$_SESSION['CompanyRecord']['decimalplaces']).'</td>
