...
<tr><td colspan="0" align="center" class="header_report">Loanee Details Report</td></tr><tr><td>&nbsp;</td></tr>
...
tbl_scheme_master.scheme_name as schemename ,tbl_sectors.sector_name,tbl_scheme_master.loan_scheme_id,tbl_scheme_master.apex_share,tbl_scheme_master.corp_share,tbl_scheme_master.promoter_share
   ...
   $output .='<table cellpadding="3" cellspacing="3" id="wrapper" class="tbl_border">';
   $output .='<tr>
 ...
 while($rs = db_fetch_object($res)){
   ...
   if($counter%2==0){ $class='header4_1';}else{$class='header4_2';}
				 ...
				 $output .='<tr>
					  ...
					  <td class="'.$class.'">'.$rs->district_name.'</td>
					  <td class="'.$class.'">'.$rs->sector_name.'</td>
					   <td class="'.$class.'">'.$rs->schemename.'</td>
					    <td class="'.$class.'">'.$rs->account_id.'</td>
						 <td class="'.$class.'">'.$rs->fname.$rs->fname.'</td>
						  <td class="'.$class.'">'.$rs->loan_amount.'</td>
						   <td class="'.$class.'">'.$rs->corp_share.'</td>
						    <td class="'.$class.'">'.$rs->apex_share.'</td>
							<td class="'.$class.'">'.$rs->promoter_share.'</td>
	            </tr>';
				$counter++;
 }
