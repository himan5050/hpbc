...
<tr><td class="header2">Name of the article</td>
...
<td class="header2">Total Amount including ST & Other Expenses</td>
...
<td class="header2">Name of person who has verified the item physically</td>
	...
	{ $counter=0;
	  ...
	  {   if($counter%2==0){ $class='header4_1';}else{$class='header4_2';}
		...
		<td class="'.$class.'">'.ucwords($r->remarks).'</td>
		<td class="'.$class.'"></td>
...
Stock Register</td></tr>
   ...
   $s="select at.code,at.date,at.quantity,im.name,im.itemrate,at.enteredby,im.openingval,at.checkedby,at.verifyphysically,at.details,at.remarks,at.billno from item_details as at,item_master as im where (at.date>='".$from."' and at.date<='".$to."') and at.code=im.code and at.code='".$itemcode."'";
...
';
	...
	{ $counter=0;
	  ...
	  $qua=$qua+$r['quantity'];
	    $output.='<tr><td class="'.$cl.'">'.ucwords($r['name']).'</td><td class="'.$cl.'" align="center">'.date('d-m-Y',$r['date']).'</td><td class="'.$cl.'">'.ucwords($r['details']).'<br>'.$r['remarks'].'</td><td class="'.$cl.'">'.$r['billno'].'</td><td class="'.$cl.'" align="right">'.$r['quantity'].'</td><td class="'.$cl.'"></td><td class="'.$cl.'" align="right">'.$qua.'</td><td class="'.$cl.'">'.ucwords($r['enteredby']).'</td></tr>';
		$counter++;
		$m++;
	  }
	}
	
	$sa="select at.itemdetails,at.date,at.quantity,im.name,im.itemrate,at.enteredby,im.openingval,at.checkedby,at.remarks,at.office from assigned_item as at,item_master as im where (at.date>='".$from."' and at.date<='".$to."') and at.itemdetails=im.code and at.itemdetails='".$itemcode."'";
	$qa=db_query($sa,$db);
	$na=1;
	if($na)
	{
	  ...
	  { 
	   ...
	   $qua=$qua-$ra['quantity']; 
	    ...
	    $output.='<tr ><td class="'.$cl.'">'.ucwords($ra['name']).'</td><td class="'.$cl.'" align="center">'.date('d-m-Y',$ra['date']).'</td><td class="'.$cl.'">Issued To: '.$myrow['locationname'].'<br>'.ucwords($ra['remarks']).'</td><td class="'.$cl.'"></td><td class="'.$cl.'"></td><td class="'.$cl.'" align="right">'.$ra['quantity'].'</td><td class="'.$cl.'" align="right">'.$qua.'</td><td class="'.$cl.'">'.ucwords($ra['enteredby']).'</td></tr>';
