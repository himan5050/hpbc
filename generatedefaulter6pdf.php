...
$expectedamount = $_REQUEST['expected'];
...
$sql = "select tbl_tehsil.tehsil_name from tbl_tehsil where tehsil_id = '".$tehsil."'";
$sqlr = db_query($sql);
$res = db_fetch_object($sqlr);
$tehsil_name = $res->tehsil_name;
...
HBCFDC KANGRA (H.P.) Defaulters List Beyond Loan Period From District:'.$district_name.' Tehsil: '.$tehsil_name.'</td></tr>
<tr><td><strong>Date  : '.date('d/m/y').'</strong></td></tr><tr><td colspan="5">- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -</td></tr>
</table>';
...
<td class="header2" width="10%"><b>Loanee & Guardian Name</b></td>
<td class="header2"><b>Scheme Name</b></td>
<td class="header2" width="12%"><b>Address</b></td>
...
<td class="header2"><b>Default Amount</b></td>
 ...
 while($res=db_fetch_array($query))
   { 
    ...
    $r1=db_fetch_array($q1);
	 ...
	 $sandate = "select UNIX_TIMESTAMP(a.sanction_date) as sdate from tbl_loan_detail a, tbl_loanee_detail b where a.reg_number=b.reg_number AND  loanee_id='".$res['loanee_id']."'";
						$asandate = db_query($sandate);
						$astb = db_fetch_array($asandate);

                        $months = floor((($enddate - $r1['start_date']) % 31556926) / 2629743.83);
                        //$expted = $months * ($res['emi_amount'] * (($res['ROI']) / 100));
					   $year1 = date('Y', $astb['sdate']);
						$year2 = date('Y');
						$month1 = date('m', $astb['sdate']);
					   $month2 = date('m');
    ...
    $output .='<tr><td class="'.$cla.'" width="3%" align="center">'.$l.'</td><td class="'.$cla.'" width="6%">'.$res['account_id'].'</td><td class="'.$cla.'"  width="10%">'.ucwords($res['fname']).' '.ucwords($res['lname']).' <br/><br/> '.ucwords($res['fh_name']).'</td><td class="'.$cla.'">'.ucwords($res['scheme_name']).'</td><td class="'.$cla.'" width="12%" style="height:65px;">'.ucwords($res['address1']).'</td><td class="'.$cla.'">'.ucwords($res['tehsil_name']).'</td><td class="'.$cla.'" align="right">'.round($opbr['opbal']).'</td><td class="'.$cla.'" align="right">'.round($opir['intpaid']).'</td><td class="'.$cla.'" align="right">'.round($oprr['recovery']).'</td><td class="'.$cla.'" align="right">'.round($expted).'</td><td class="'.$cla.'" align="right">'.abs(round($defaultamount)).'</td><td class="'.$cla.'" align="right">'.$res['o_principal'].'</td></tr>';
	   ...
	   $l++;
						...
						<td colspan="11">- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -</td></tr ><tr >
                         ...
                         <td  width="8%"><strong>'.$open_bal.'</strong></td> 
                         <td  width="8%"><strong>'.$int_paid.'</strong></td>
