...
<div id="rec_participant">
  ...
  <tr>	<td align="left" class="tdform-width"><fieldset><legend>Interest Subsidy Report</legend>
  ...
  <tr>
 <tr><td colspan="7" align="right"><div style="margin-right: 80px;"><?php print drupal_render($form); ?></div></td></tr>
...
$cond = '';	
  ...
  $res = pager_query($query, 20, 0, $count_query);

 
   $pdfurl = $base_url."/interestsubsidyReportpdf.php?op=interestsubsidyReport_report";
   ...
   $pdfimage = $base_url.'/'.drupal_get_path('theme','scst')."/images/pdf_icon.gif";
	...
	<tr class="oddrow"><td align="left" colspan="15"><h2 style="text-align:left">Interest Subsidy Report</h2></td></tr>
	<tr>
   ...
   while($rs = db_fetch_object($res)){
	...
	/*   $sqlh = "select disbursed_amount from tbl_loan_detail where loanee_id='".$rs->loanee_id."'";
	 $resh = db_query($sqlh);
...
$sumamount =$sqlqu->sumamount;
	...
	if ($counter%2==0){$cl="even";}else{$cl="odd";}
	  $output .='<tr class="'.$cl.'">
					  ...
					  <td align="right">'.round(abs($rs->amountdis)).'</td>
					   
					  <td align="right">'.round(abs($rs->interestamount)).'</td>
					 <td align="left">'.ucwords(getBankName($rs->bank_id)).'</td>
					 <td align="left">'.ucwords($rs->bankbranch_name).'</td>
 ...
 $output .='</table></div>';
   echo $output .= theme('pager', NULL, 10, 0);
	 ...
	 form_set_error('from','To date should be greater from date');
