...
$append="";
	...
	$append .= " action='".$status."' AND ";
	}
	if($_REQUEST['employee_id']){
	
	$append .= " employee_id='".$employee_id."' AND ";
	}
	if($_REQUEST['employee_name']){
	
	$append .= " employee_name='".$employee_name."' AND ";
	}
	 if($_REQUEST['Departmentid']){
	
	$append .= " current_Departmentid='".$section."' AND ";
	}
...
//$assigned_to=db_fetch_object($assign);
   ...
   if($counter%2==0){ $class='header4_1';}else{$class='header4_2';}
				...
				<td width="8%" class="'.$class.'" align="right">'.ucwords($rs->employee_name).'</td>
				...
				<td width="8%" class="'.$class.'" align="center">'.ucwords(getLookupName($rs->current_Departmentid)).'</td>
				<td width="8%" class="'.$class.'" align="center">'.ucwords(getLookupName($rs->current_designationid)).'</td>
				<td width="8%" class="'.$class.'" align="center">'.ucwords(getCorporationName($rs->prev_officeid)).'</td>
				<td width="8%" class="'.$class.'" align="center">'.ucwords(getLookupName($rs->prev_Departmentid)).'</td>
