...
<td class="header2">Status</td>
	...
	if($n)
	{
	  ...
	  {
	    $cl="header4_1";
		  $output .='<tr ><td class="header4_1">'.$qua.'</td><td class="header4_1">'.$r['emp_id'].'</td><td class="header4_1">'.ucwords($r['employee_name']).'</td><td class="header4_1">Medical</td><td class="header4_1" align="center">'.date('d-m-Y',$r['date']).'</td><td class="header4_1" align="right">'.round($r['net_amount']).'</td><td  class="header4_1">'. $st.'</td></tr>';
	  }
	  else
	  {
	    $cl="header4_2";
		$output .='<tr ><td class="header4_2">'.$qua.'</td><td class="header4_2">'.$r['emp_id'].'</td><td class="header4_2">'.ucwords($r['employee_name']).'</td><td class="header4_2">Medical</td><td class="header4_2" align="center">'.date('d-m-Y',$r['date']).'</td><td class="header4_2" align="right">'.round($r['net_amount']).'</td><td class="header4_2">'.$st.'</td></tr>';
	  } 
	...
	if($qua%2==0)
	  ...
	  else
	  {
	    ...
	    $output.='<tr ><td class="'.$cl.'">'.$qua.'</td><td class="'.$cl.'">'.$ra['emp_id'].'</td><td class="'.$cl.'">'.ucwords($ra['employee_name']).'</td><td class="'.$cl.'">Tour</td><td class="'.$cl.'" align="center">'.date('d-m-Y',$ra['date']).'</td><td class="'.$cl.'" align="right">'.round($ra['total_amount']).'</td><td class="'.$cl.'">'.$st.'</td></tr>';
	  }
	}
	  }
...
if($qr['amount']>0)
{
  $debtotal=$debtotal+$qr['amount'];
	...
	$data .= '<tr>
				...
				<td class="'.$cla.'" align="right">'.round(abs($qr['amount'])).'</td>
		  </tr>';
		   $f++;
     }
...
$data.= '<tr><td>Total</td><td>&nbsp;</td>
				<td align="right">'.round(abs($debtotal)).'</td></tr>';
$data .='</table>';
...
if($qre['amount']<0)
{
  $debtotale=$debtotale+$qre['amount'];
	...
	$datae .= '<tr>
				...
				<td class="'.$cla.'" align="right">'.round(abs($qre['amount'])).'</td>
		  </tr>';
		  $fe++;
     }
...
$datae .= '<tr><td>Total</td><td>&nbsp;</td><td align="right">'.round(abs($debtotale)).'</td></tr>';
 $datae .='</table>';
...
$bank=array();
 ...
 $opbr=db_fetch_array($opbq);
  ...
  $s="SELECT gltrans.*,amount as bamount,chartmaster.accountname FROM gltrans, chartmaster WHERE gltrans.account=chartmaster.accountcode AND gltrans.trandate ='".$from."'";
 ...
 //$s="SELECT DISTINCT(gltrans.voucher_no), banktrans.ref, gltrans.account,banktrans.banktranstype, banktrans.amount as bamount, banktrans.transno, gltrans.chequeno,chartmaster.accountname FROM banktrans,gltrans,chartmaster WHERE banktrans.transdate ='".$from."'and banktrans.transno=gltrans.typeno and banktrans.transdate=gltrans.trandate and gltrans.account=chartmaster.accountcode ";
	...
	<tr><td colspan="12" class="header2">Opening Balance:'.round(($opbr['totamount']+$acto)).' </td></tr>
	...
	//echo $r['account'];
		 ...
		 if($r['type'] == 12)
		  {
			...
			$output .='<tr ><td class="'.$cl.'">'.$r["voucher_no"].'</td><td class="'.$cl.'">'.$r['account'].'</td><td class="'.$cl.'">'.$r["accountname"].'</td><td class="'.$cl.'">'.$r["narrative"].'</td><td class="'.$cl.'">'.$cash.'</td><td class="'.$cl.'">'.$cheque.'</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td></tr>';
			$nn++;
		}
		...
		else if($r['bamount']>0)
	  {
		 ...
		 if($r['type'] == 1)
			...
			$output .='<tr><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">--</td><td class="'.$cl.'">'.$r["voucher_no"].'</td><td class="'.$cl.'">'.$r['account'].'</td><td class="'.$cl.'">'.$r["accountname"].'</td><td class="'.$cl.'">'.$r["narrative"].'</td><td class="'.$cl.'">'.$cash.'</td><td class="'.$cl.'">'.$cheque.'</td></tr>';
			$nn++;
		}
	  //}
	}	
...
//$pdf->writeHTML($output, true, 0, true, true);
	...
	$output .='</b></td>
			<td class="header2"><b>'.round($Balance*$ExRate,2).'</b></td></tr>';
...
$DisplayTranDate=$DisplayTranDat[2]."-".$DisplayTranDat[1]."-".$DisplayTranDat[0];
		...
		$output .='<tr><td align="center" class="'.$cla.'">'.$DisplayTranDate.'</td>
				...
				<td class="'.$cla.'" align="right">'.round(abs($myrow1['amt'])).'</td>
				<td class="'.$cla.'" align="right">'.round(abs($myrow1['outstanding'])).'</td>
				</tr>';
	...
	//end of while loop
	$output .='<tr><td colspan="5" class="header4_2"><b>Total of all unpresented cheques</b></td><td align="right" class="header4_2">'. round(abs($TotalUnpresentedCheques)) .'</td></tr>';
	...
	else
	{
	  $cla="header4_2";
		...
		$output .='<tr><td align="center"  class="'.$cla.'">'.$DisplayTranDate.'</td>
				...
				<td class="'.$cla.'" align="right">'.round(abs($myrow['amt'])).'</td>
				<td class="'.$cla.'" align="right">'.round(abs($myrow['outstanding'])).'</td>
				</tr>' ;
	...
	$output .='
			<tr >
				...
				<td align="right" class="header4_1">' . round(abs($TotalUnclearedDeposits)). '</td><td class="header4_1"></td></tr>';
	...
	$output .='
			<tr >
				...
				<td align="right" class="header2">' . round(abs($FXStatementBalance)). '</td><td class="header4_1"></td></tr>';
			...
			if ($ActGrp !=''){ //so its not the first account group of the first account displayed
				if ($myrow['parentgroupname']==$ActGrp){
					...
					$ParentGroups[$Level]='';
						...
						$gactual = ($GrpActual[$Level] < 0)?'Dr':'Cr';
						$gbudget = ($GrpBudget[$Level] < 0)?'Dr':'Cr';
						$gppactual = ($GrpPrdActual[$Level] < 0)?'Dr':'Cr';
						$gpbudget = ($GrpPrdBudget[$Level] < 0)?'Dr':'Cr';
					$output .='<tr>
						<td colspan="2"><b>'.$ParentGroups[$Level].' Total </b></td>
						<td align="right"><b>'.round(abs($GrpActual[$Level])).$gactual.'</b></td>
						<td align="right"><b>'.round(abs($GrpBudget[$Level])).$gbudget.'</b></td>
						<td align="right"><b>'.round(abs($GrpPrdActual[$Level])).$gppactual.'</b></td>
						<td align="right"><b>'.round(abs($GrpPrdBudget[$Level])).$gpbudget.'</b></td>
						</tr>';
...
if($j%2==0)
	...
	else
	{
	 $cl="header4_2";
		...
		$mactual = ($myrow['monthactual'] < 0)?'Dr':'Cr';
		$mbudget = ($myrow['monthbudget'] < 0)?'Dr':'Cr';
		$pactual = ($AccountPeriodActual < 0)?'Dr':'Cr';
		$pbudget = ($AccountPeriodBudget < 0)?'Dr':'Cr';
		$output .='<tr><td class="'.$cl.'">'.$ActEnquiryURL.'</td>
			<td class="'.$cl.'">'.$myrow['accountname'].'</td>
			<td class="'.$cl.'" align="right">'.round(abs($myrow['monthactual'])).$mactual.'</td>
			<td class="'.$cl.'" align="right">'.round(abs($myrow['monthbudget'])).$mbudget.'</td>
			<td class="'.$cl.'" align="right">'.round(abs($AccountPeriodActual)).$pactual.'</td>
			<td class="'.$cl.'" align="right">'.round(abs($AccountPeriodBudget)).$pbudget.'</td>
			</tr>';
	...
	if ($ActGrp !=''){ //so its not the first account group of the first account displayed
		...
		$cmactual = ($CheckMonth < 0)?'Dr':'Cr';
		$cmbudget = ($CheckBudgetMonth < 0)?'Dr':'Cr';
		$cpactual = ($CheckPeriodActual < 0)?'Dr':'Cr';
		$cpbudget = ($CheckPeriodBudget < 0)?'Dr':'Cr';

	$output .='<tr bgcolor="#ffffff">
			<td colspan="2" class="header2"><b> Check Totals</b></td>
			<td class="header2" align="right">'.round(abs($CheckMonth)).$cmactual.'</td>
			<td class="header2" align="right">'.round(abs($CheckBudgetMonth)).$cmbudget.'</td>
			<td class="header2" align="right">'.round(abs($CheckPeriodActual)).$cpactual.'</td>
			<td class="header2" align="right">'.round(abs($CheckPeriodBudget)).$cpbudget.'</td>
		</tr>';
...
//	$ErrMsg = _('Could not retrieve the currency and exchange rate for the selected bank account');
	...
	$output .='</b></td>
			<td valign="bottom" class="number"><b>' . round($Balance*$ExRate,2) . '</b></td></tr>';
...
$DisplayTranDate=$DisplayTranDat[0]."-".$DisplayTranDat[1]."-".$DisplayTranDat[2];
		$output .='<td align="center">'.$DisplayTranDate.'</td>
				...
				<td align="right" class="number">'.round($myrow['amt']).'</td>
				<td align="right" colspan="2" class="number">'.round($myrow['outstanding']).'</td>
				</tr>';
	...
	//end of while loop
	$output .='<tr></tr>
			<tr class="even"><td colspan="6"><b>Total of all unpresented cheques</b></td><td align="right" class="number">' . round(abs($TotalUnpresentedCheques)) . '</td></tr>';
	...
	$output .='<tr></tr>
			<tr class="oddrow">
				...
				<td align="right" class="number">' . round($FXStatementBalance) . '</td></tr>';
...
//	echo '<div class="breadcrumb">Home &raquo; <a href="'.$_SERVER['PHP_SELF'].'">Balance Sheet</a></div>';
   $bald=explode('-',$BalanceDate);
	...
	while ($myrow=db_fetch_array($AccountsResult)) {
			...
			if ($myrow['parentgroupname']!=$ActGrp){
				while ($myrow['groupname']!=$ParentGroups[$Level] AND $Level>0){
					...
					$output .='<tr><td colspan="2">'.$ParentGroups[$Level].'</td>
						<td >'.round($GroupTotal[$Level]).'</td>
						<td></td>
						<td >'.round($LYGroupTotal[$Level]).'</td>
						</tr>';
					$GroupTotal[$Level] = 0;
				...
				$output .='<tr><td colspan="2">'.$ParentGroups[$Level].'</td>
					<td class=number>'.round($GroupTotal[$Level]).'</td>
					<td></td>
					<td class=number>'.round($LYGroupTotal[$Level]).'</td>
					</tr>';
				$GroupTotal[$Level] = 0;
				$LYGroupTotal[$Level] = 0;
				$ParentGroups[$Level]='';
		...
		if ($myrow['sectioninaccounts']!= $Section ){
			...
			if ($Section!=''){
				...
				$output .='<tr>
					...
					<td >'.round($SectionBalance).'</td>
					<td></td>
					<td >'.round($SectionBalanceLY).'</td>
				</tr>';
				$j++;
			}
			$SectionBalanceLY = 0;
		...
		//$cla="header4_2";
			...
			$output .='<tr><td class="'.$cla.'">'.$ActEnquiryURL.'</td>
					...
					<td class="'.$cla.'">'.round($AccountBalance).'</td>
					<td class="'.$cla.'"></td>
					<td class="'.$cla.'">'.round($LYAccountBalance).'</td>
					<td class="'.$cla.'"></td>
					</tr>';
	...
	$output .='<tr>
		...
		<td class="header2">'.round($CheckTotal).'</td>
		<td class="header2"></td>
		<td class="header2">'.round($LYCheckTotal).'</td>
		</tr>';
...
if($daa[1]=='12')
{
 $mo="December";
			...
			if ($myrow['parentgroupname']!=$ActGrp AND $ActGrp!=''){
				...
				if ($Section ==1){ /*Income */
						$output .='<tr>
								...
								<td></td>
								<td align="right">'.round(-$GrpPrdActual[$Level]).'</td>
								<td></td>
								<td align="right">'.round(-$GrpPrdBudget[$Level]).'</td>
								<td></td>
								<td align="right">'.round(-$GrpPrdLY[$Level]).'</td>
								</tr>';
					} else { /*Costs */
						$output .='<tr>
								<td colspan="2">'.$ActGrpLabel.'</td>
			...
			if ($SectionPrdLY+$SectionPrdActual+$SectionPrdBudget !=0){
				if ($Section==1) { /*Income*/
					...
					$output .='<tr>
							...
							<td></td>
							<td align="right">'.round(-$SectionPrdActual).'</td>
							<td></td>
							<td align="right">'.round(-$SectionPrdBudget).'</td>
							<td></td>
							<td align="right">'.round(-$SectionPrdLY).'</td>
							</tr>';
				...
				if ($Section==2){ /*Cost of Sales - need sub total for Gross Profit*/
					...
					$output .='<tr>
							...
							<td></td>
							<td align="right">'.round($TotalIncome - $SectionPrdActual).'</td>
							<td></td>
							<td align="right">'.round($TotalBudgetIncome - $SectionPrdBudget).'</td>
							<td></td>
							<td align="right">'.round($TotalLYIncome - $SectionPrdLY).'</td>
							</tr>';
					...
					$output .='<tr>
							...
							<td></td>
							<td align="right">'.round($PrdGPPercent,1) . '%</td>
							<td></td>
							<td align="right">'.round($BudgetGPPercent,1) . '%</td>
							<td></td>
							<td align="right">'.round($LYGPPercent,1). '%</td>
		  ...
		  // $cla="header4_2";
			...
			if ($Section ==1){
				 $output .='<tr><td class="'.$cla.'">'.$ActEnquiryURL.'</td>
						<td class="'.$cla.'">'.$myrow['accountname'].'</td>
						<td class="'.$cla.'"></td>
						<td class="'.$cla.'" align="right">'.round(-$AccountPeriodActual).'</td>
						<td class="'.$cla.'"></td>
						<td class="'.$cla.'" align="right">'.round(-$AccountPeriodBudget).'</td>
						<td class="'.$cla.'"></td>
						<td class="'.$cla.'" align="right">'.round(-$AccountPeriodLY).'</td>
						</tr>';
			} else {
				$output .='<tr><td class="'.$cla.'">'.$ActEnquiryURL.'</td>
						<td class="'.$cla.'">'.$myrow['accountname'].'</td>
	...
	$output .='<tr bgcolor="#ffffff">
			...
			<td></td>
			<td align="right">'.round(-$PeriodProfitLoss).'</td>
			<td></td>
			<td align="right">'.round(-$PeriodBudgetProfitLoss).'</td>
			<td></td>
			<td align="right">'.round(-$PeriodLYProfitLoss).'</td>
			</tr>';
	...
	$output .='<tr>
			...
			<td></td>
			<td align="right">'.round($PrdNPPercent,1) . '%</td>
			<td></td>
			<td align="right">'.round($BudgetNPPercent,1) . '%</td>
			<td></td>
			<td align="right">'.round($LYNPPercent,1). '%</td>
...
<tr><td class="header1"><b>From Date :</b> '. $fr[2].'-'.$fr[1].'-'.$fr[0].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>To Date :</b> '.$tr[2].'-'.$tr[1].'-'.$tr[0].'</td></tr>
 ...
 if($r['amount']>0)
   ...
   $fro=explode('-',$r['trandate']);
	if(($r['type'] == 12 && $r['amount']<0) || ($r['type'] == 1 && $r['amount']>0))
	{
		$output .='<tr ><td class="'.$cla.'" align="center">'.$n.'</td><td align="center" class="'.$cla.'">'. $fro[2].'-'.$fro[1].'-'.$fro[0].'</td><td class="'.$cla.'" align="right">'.$r['chequeno'].'</td><td class="'.$cla.'">'.$typ.'</td><td class="'.$cla.'" align="right">'.round(abs($r['amount'])).'</td><td class="'.$cla.'">'.$r['narrative'].'</td></tr>';
		$n++;
   }
 }
	...
	while ($myrow=db_fetch_array($TransResult)) {
			...
			//	$ErrMsg = _('The chart details for account') . ' ' . $SelectedAccount . ' ' . _('could not be retrieved');
			   ...
			   $lastdate=$lasd[2].'-'.$lasd[1].'-'.$lasd[0];
				...
				if ($PeriodTotal < 0 ){ //its a credit balance b/fwd
					...
					$output .='<td></td>
						<td ><b>' . round(abs(-$PeriodTotal)) . '</b></td>
						<td></td>
						</tr>';
				} else { //its a debit balance b/fwd
					if ($PandLAccount==True) {
					...
					$output .='<td ><b>' . round(abs($PeriodTotal)) . '</b></td>
						<td colspan="2"></td>
						</tr>';
				}
				/*$IntegrityReport .= '<br />Period: ' . $PeriodNo  . 'Account movement per transaction: '  . round($PeriodTotal) . ' Movement per ChartDetails record : ' . round($ChartDetailRow['actual']) . ' Period difference : ' . round($PeriodTotal -$ChartDetailRow['actual'],3);*/
      ...
      $URL_to_TransDetail='';
		...
		$output .='<tr><td width="10%" class="'.$cla.'">'.$myrow['voucher_no'].'</td>
			...
			<td align="right" class="number" class="'.$cla.'"><b>'.round(abs($RunningTotal)).'</b></td>
